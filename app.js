...
function renderMarketPulse(){
  ...
  list.innerHTML=events.length?events.map(event=>{const matches=marketMatches(event),progress=marketSessionProgress(event,matches),remaining=Math.max(0,progress.total-progress.workedIds.size),marketFollowUps=matches.filter(person=>marketTriggeredFollowUp(event,person.id)).length,recency=relativeEventRecency(event),priority=hotSpottingPriority(event,matches.length),buttonLabel=progress.active?'Active Session':'Start Session',buttonClass=progress.active?'primary market-session-active-btn':'primary market-session-start-btn',details=`${marketEventDetailHtml(event)}${marketFollowUps?`<div class="market-followup-ready"><strong>${marketFollowUps}</strong><span>requested market follow-up${marketFollowUps===1?'':'s'} ready first</span></div>`:''}<div class="hotspot-opportunity-summary"><span><strong>${remaining}</strong> remaining</span><span><strong>${formatEstimatedTime(estimatedMinutes(remaining))}</strong> session</span><span class="priority-${priority.label.toLowerCase()}"><strong>${priority.label}</strong> priority</span><span><strong>${recency.label}</strong> event</span></div>`;if(progress.complete)return`<article class="market-pulse-card market-session-complete-card" data-market-event-id="${escapeHtml(event.id)}"><div class="market-pulse-card-head"><span class="market-event-tag event-${marketTypeClass(event.eventType)}">${escapeHtml(event.eventType)}</span><button type="button" data-remove-market-event="${escapeHtml(event.id)}" aria-label="Remove event">×</button></div><details class="market-pulse-completed-disclosure"><summary><span><strong>${escapeHtml(event.address)}</strong><small>${escapeHtml(event.suburb)}</small></span><span class="market-pulse-complete-state"><strong>Complete</strong><i aria-hidden="true">›</i></span></summary><div class="market-pulse-card-content">${details}</div></details></article>`;return`<article class="market-pulse-card${progress.active?' market-session-active-card':''}" data-market-event-id="${escapeHtml(event.id)}"><div class="market-pulse-card-head"><span class="market-event-tag event-${marketTypeClass(event.eventType)}">${escapeHtml(event.eventType)}</span><button type="button" data-remove-market-event="${escapeHtml(event.id)}" aria-label="Remove event">×</button></div><div class="market-pulse-card-title"><h4>${escapeHtml(event.address)}</h4><p>${escapeHtml(event.suburb)}</p></div><div class="market-pulse-card-content">${details}<button class="${buttonClass}" type="button" data-start-market-session="${escapeHtml(event.id)}" ${matches.length?'':'disabled'}>${matches.length?buttonLabel:'No Matching Contacts'}</button></div></article>`}).join(''):'<div class="prospect-empty"><strong>No hot spotting opportunities yet</strong><small>Paste today’s email below, then tap Find Opportunities.</small></div>';
}
...
async function upsertBuyer(data,id='',context=null){
  ...
  return{record,reusedExisting:Boolean(existing),addedBuyerRole:Boolean(existing&&!wasBuyer),unified:resolution.unified}
}
...
async function upsertProspect(data,id=''){
  const resolution=resolveBuyerCanonical(data,id),existing=resolution.record,createdAt=existing?.createdAt||Date.now(),dataCreditedAt=existing?.dataCreditedAt||(!existing?createdAt:0),buyerLinked=prospectHasActiveBuyerRole(existing),positionTags=buyerLinked?Array.from(new Set(['Buyer Seller',...buyerPositionTags(existing)])):buyerPositionTags(existing),record=normaliseProspect({...existing,...data,id:existing?.id||prospectId(),sellerProfileActive:Boolean(existing?.sellerProfileActive||existing?.recordType==='buyer'),buyerPositionTags:positionTags,buyerSeller:buyerLinked||existing?.buyerSeller,dataCreditedAt,createdAt,updatedAt:Date.now()});
  if(existing)prospects=prospects.map(p=>p.id===record.id?record:p);else prospects.unshift(record);
  if(existing&&existing.sellingTimeframe!==record.sellingTimeframe)prospectInteractions.push({id:prospectId(),prospectId:record.id,date:todayKey(),at:Date.now(),type:'Pipeline',outcome:'Selling timeframe updated',note:`Selling timeframe changed from ${existing.sellingTimeframe||'Not set'} to ${record.sellingTimeframe||'Not currently selling'}.`,nextFollowUp:''});
  activeProspectId=record.id;
  if(!id){
    saveProspectingLocal();
    saveProspecting({render:false,awaitCloud:false}).catch(err=>console.error('New contact background sync failed',err));
    if(!existing)creditNewProspectData(record).catch(err=>console.error('New contact Data credit is saved locally and pending sync',err));
  }else{
    await saveProspecting({render:false});
  }
  setProspectorSection('contacts');renderProspecting();$('#prospectingDashboard').classList.add('hidden');$('#prospectDetail').classList.remove('hidden');renderProspectDetail(record.id);
  return{record,reusedExisting:Boolean(existing),unified:resolution.unified}
}
...
$('#prospectingView').onsubmit=async e=>{
  ...
  if(e.target.id==='prospectEditor'){
    e.preventDefault();const form=e.target,submit=form.querySelector('button[type=submit]'),f=new FormData(form),existing=prospectById(activeProspectId),wasExisting=Boolean(existing),addBuyerBrief=f.get('addBuyerBrief')==='1';if(submit?.disabled)return;if(submit){submit.disabled=true;submit.textContent=wasExisting?'Saving…':'Adding…'}
    try{const result=await upsertProspect({name:f.get('name'),phone:f.get('phone'),email:f.get('email'),address:f.get('address'),source:f.get('source'),stage:f.get('stage'),temperature:f.get('temperature'),motivation:f.get('motivation'),temperatureManual:form.dataset.temperatureManual==='1'||Boolean(existing?.temperatureManual),motivationManual:form.dataset.motivationManual==='1'||Boolean(existing?.motivationManual),tags:f.get('tags'),sellingTimeframe:f.get('sellingTimeframe'),nextFollowUp:f.get('nextFollowUp'),notes:f.get('notes'),archived:Boolean(existing?.archived),archivedAt:existing?.archivedAt||0},activeProspectId||'');if(addBuyerBrief){openBuyerEditor(result.record.id);toast('Contact saved. Add their buyer brief.')}else toast(result.unified?'Buyer and seller records unified':result.reusedExisting&&!wasExisting?'Existing contact reused · no duplicate created':wasExisting?'Contact saved':'Contact added')}
    catch(err){console.error('Contact save failed',err);toast('Contact saved on this device. Please check sync.');if(submit){submit.disabled=false;submit.textContent=wasExisting?'Save Contact':'Add Contact'}}
    return
  }
