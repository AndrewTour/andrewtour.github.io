...
function coachingEngine(viewDate=selectedDate,items=timelineItemsForDate(viewDate)){
  ...
  if(currentAppointment){const intel=appointmentIntelligenceFocusMeta(currentAppointment.appointment,currentAppointment.appointment?.createdDate||viewDate);return{title:'Appointment Window',meta:intel?`${intel} · Resume prospecting afterwards`:`${currentAppointment.title} · Resume prospecting afterwards`,focusItemId:currentAppointment.id};}
  if(nextAppointment&&minutesToAppointment<=10){const intel=appointmentIntelligenceFocusMeta(nextAppointment.appointment,nextAppointment.appointment?.createdDate||viewDate);return{title:'Appointment Window',meta:intel?`${intel} · Starts in ${minutesToAppointment} min`:`${nextAppointment.title} starts in ${minutesToAppointment} min`,focusItemId:nextAppointment.id};}
  if(nextAppointment&&minutesToAppointment<=30){const intel=appointmentIntelligenceFocusMeta(nextAppointment.appointment,nextAppointment.appointment?.createdDate||viewDate);return{title:'Appointment Window',meta:intel?`${intel} · Prepare for ${timelineTimeLabel(nextAppointment.minutes)}`:`Complete the current block, then prepare for ${timelineTimeLabel(nextAppointment.minutes)}`,focusItemId:nextAppointment.id};}
...
const APPOINTMENT_MARKET_WINDOW_DAYS=90;
function marketPropertyTypeFromDetails(details=''){
  const matches=cleanMarketLine(details).match(/\b(?:house|duplex|semi[- ]detached|terrace|unit|apartment|villa|townhouse|land)\b/gi)||[];
  return matches.length?matches.at(-1).toLowerCase().replace(/\s+/g,' '):'';
}
function marketPropertyGroupFromType(type=''){
  const value=normalisePlace(type);
  if(['house','duplex','semi detached','terrace'].includes(value))return'house';
  if(['unit','apartment','villa','townhouse'].includes(value))return'strata';
  if(value==='land')return'land';
  return'';
}
function inferredPropertyGroupFromAddress(address=''){
  const clean=cleanMarketLine(address).split(',')[0].trim();
  if(!clean)return'';
  const first=clean.split(/\s+/)[0]||'';
  if(/^\d+[a-b]$/i.test(first))return'house';
  if(clean.includes('/')||/^\d+\s*[-–]\s*\d+\b/.test(clean)||/^\d+[a-z]?\s*[-–]\s*\d+/i.test(first))return'strata';
  return'house';
}
function appointmentAddressParts(value=''){
  const clean=cleanMarketLine(value),split=splitMarketAddress(clean);
  if(split.suburb)return split;
  const lower=clean.toLowerCase(),suburb=[...SYDNEY_SUBURBS].sort((a,b)=>b.length-a.length).find(name=>lower.endsWith(name.toLowerCase()));
  if(!suburb)return{address:clean,suburb:''};
  return{address:clean.slice(0,clean.length-suburb.length).replace(/[ ,]+$/,'').trim(),suburb};
}
function marketEventPropertyGroup(event){return marketPropertyGroupFromType(marketPropertyTypeFromDetails(event?.propertyDetails))||inferredPropertyGroupFromAddress(event?.address)}
function marketEventAgeDays(event){const date=validDateKey(event?.receivedDate)?parseKey(event.receivedDate):new Date(Number(event?.createdAt)||0);return Number.isFinite(date.getTime())?Math.max(0,Math.floor((parseKey(todayKey())-date)/86400000)):9999}
function samePropertyAddress(event,address,suburb){return normalisePlace(event?.address)===normalisePlace(address)&&normalisePlace(event?.suburb)===normalisePlace(suburb)}
function appointmentPropertyContext(address=''){
  const parts=appointmentAddressParts(address),recent=normaliseMarketPulseEvents(marketPulseEvents).filter(event=>marketEventAgeDays(event)<=APPOINTMENT_MARKET_WINDOW_DAYS),exact=recent.filter(event=>samePropertyAddress(event,parts.address,parts.suburb)),explicit=exact.map(event=>marketPropertyGroupFromType(marketPropertyTypeFromDetails(event.propertyDetails))).find(Boolean),group=explicit||inferredPropertyGroupFromAddress(parts.address||address);
  return{...parts,group,source:explicit?'MarketPulse':'Address pattern'};
}
function dedupeAppointmentMarketEvents(events=[]){
  const sorted=[...events].sort((a,b)=>(Number(b.createdAt)||0)-(Number(a.createdAt)||0)),seen=new Set();
  return sorted.filter(event=>{const key=`${normalisePlace(event.address)}|${normalisePlace(event.suburb)}|${normalisePlace(event.eventType)}`;if(seen.has(key))return false;seen.add(key);return true});
}
function buyerPropertyGroup(p){return marketPropertyGroupFromType(p?.buyerPropertyType)}
function appointmentMatchingBuyers(suburb='',group=''){
  const place=normalisePlace(suburb);if(!place)return[];
  return activeBuyerProspects().filter(p=>p.buyerStage!=='Purchased'&&(p.buyerSuburbs||[]).some(value=>normalisePlace(value)===place)&&(!p.buyerPropertyType||!group||buyerPropertyGroup(p)===group));
}
function appointmentIntelligence(appointment={},sourceDate=''){
  const context=appointmentPropertyContext(appointment.address||''),place=normalisePlace(context.suburb),events=dedupeAppointmentMarketEvents(normaliseMarketPulseEvents(marketPulseEvents).filter(event=>marketEventAgeDays(event)<=APPOINTMENT_MARKET_WINDOW_DAYS&&normalisePlace(event.suburb)===place&&(!context.group||marketEventPropertyGroup(event)===context.group))),buyers=appointmentMatchingBuyers(context.suburb,context.group),sold=events.filter(event=>normalisePlace(event.eventType)==='sold'),active=events.filter(event=>['just listed','price update','under offer'].includes(normalisePlace(event.eventType))),groupLabel=context.group==='strata'?'Strata':context.group==='house'?'House / duplex':context.group==='land'?'Land':'Property';
  return{...context,groupLabel,events,buyers,soldCount:sold.length,activeCount:active.length,scheduledDate:appointmentScheduledDate(appointment,sourceDate||appointment.createdDate||todayKey())};
}
function appointmentIntelligenceFocusMeta(appointment={},sourceDate=''){
  const intel=appointmentIntelligence(appointment,sourceDate),bits=[];
  if(intel.suburb)bits.push(intel.suburb);
  if(intel.groupLabel!=='Property')bits.push(intel.groupLabel.toLowerCase());
  if(intel.soldCount)bits.push(`${intel.soldCount} sold`);
  if(intel.activeCount)bits.push(`${intel.activeCount} active`);
  if(intel.buyers.length)bits.push(`${intel.buyers.length} matching buyer${intel.buyers.length===1?'':'s'}`);
  return bits.join(' · ');
}
function renderAppointmentIntelligence(){
  const panel=$('#appointmentIntelligence');if(!panel)return;
  if(!editingAppointment){panel.classList.add('hidden');return}
  const sourceDate=editingAppointment.sourceDate,appointment=dayData(sourceDate).appointments.find(a=>String(a.id)===String(editingAppointment.id));
  if(!appointment){panel.classList.add('hidden');return}
  const draft={...appointment,address:$('#appointmentAddress')?.value||appointment.address},intel=appointmentIntelligence(draft,sourceDate),eventLabel=intel.events.length===1?'market event':'market events';
  panel.classList.remove('hidden');
  $('#appointmentIntelligenceTitle').textContent=intel.suburb?`${intel.suburb} market context`:'Local market context';
  $('#appointmentIntelligenceType').textContent=`${intel.groupLabel} · ${intel.source}`;
  $('#appointmentIntelligenceSummary').textContent=intel.suburb?`${intel.events.length} recent matching ${eventLabel} from MarketPulse, grouped to the same property category. Buyer depth uses active briefs targeting ${intel.suburb}.`:'Add a suburb to the property address to match MarketPulse activity and buyer demand.';
  $('#appointmentIntelligenceStats').innerHTML=[['Sold',intel.soldCount,'Recent matching results'],['Active',intel.activeCount,'Listings / price activity'],['Buyers',intel.buyers.length,'Compatible active briefs']].map(([label,value,meta])=>`<div class="appointment-intelligence-stat"><span>${label}</span><strong>${value}</strong><small>${meta}</small></div>`).join('');
  const evidence=intel.events.slice(0,4);
  $('#appointmentIntelligenceEvidence').innerHTML=evidence.length?evidence.map(event=>{const value=event.price||event.guide||event.daysOnMarket||'',type=marketPropertyTypeFromDetails(event.propertyDetails),meta=[value,type?type.charAt(0).toUpperCase()+type.slice(1):'',relativeEventRecency(event).label].filter(Boolean).join(' · ');return`<div class="appointment-intelligence-event"><span>${escapeHtml(event.eventType)}</span><div><strong>${escapeHtml(event.address)}</strong><small>${escapeHtml(meta)}</small></div></div>`}).join(''):'<div class="appointment-intelligence-event"><span>MarketPulse</span><div><strong>No matching activity yet</strong><small>AGNT will populate this as matching suburb and property-type events arrive.</small></div></div>';
}
...
$('#appointmentContactName').addEventListener('input',e=>{const selected=appointmentLinkedProspectId?prospectById(appointmentLinkedProspectId):null;if(selected&&e.target.value.trim()!==selected.name)appointmentLinkedProspectId='';renderAppointmentContactSuggestions()});$('#appointmentAddress').addEventListener('input',()=>{if(editingAppointment)renderAppointmentIntelligence()});$('#appointmentContactName').addEventListener('focus',renderAppointmentContactSuggestions);$('#appointmentContactSuggestions').addEventListener('click',e=>{const b=e.target.closest('[data-appointment-contact]');if(!b)return;selectAppointmentContact(b.dataset.appointmentContact)});$('#cancelProspectAppointmentFlow').onclick=cancelProspectAppointmentFlow;document.addEventListener('pointerdown',e=>{if(!e.target.closest('.appointment-contact-picker'))hideAppointmentContactSuggestions()});
