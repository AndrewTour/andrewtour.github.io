...
function renderMarketPulse(){
  ...
  list.innerHTML=events.length?events.map(event=>{
    ...
    if(progress.complete)return`<article class="market-pulse-card market-session-complete-card" data-market-event-id="${escapeHtml(event.id)}"><div class="market-pulse-card-head"><span class="market-event-tag event-${marketTypeClass(event.eventType)}">${escapeHtml(event.eventType)}</span><button type="button" data-remove-market-event="${escapeHtml(event.id)}" aria-label="Remove event">×</button></div><details class="market-pulse-completed-disclosure"><summary><span><strong>${escapeHtml(event.address)}</strong><small>${escapeHtml(event.suburb)}</small></span><span class="market-pulse-complete-state"><strong>Complete</strong><i aria-hidden="true">›</i></span></summary><div class="market-pulse-card-content">${details}</div></details></article>`;
    return`<article class="market-pulse-card${progress.active?' market-session-active-card':''}" data-market-event-id="${escapeHtml(event.id)}"><div class="market-pulse-card-head"><span class="market-event-tag event-${marketTypeClass(event.eventType)}">${escapeHtml(event.eventType)}</span><button type="button" data-remove-market-event="${escapeHtml(event.id)}" aria-label="Remove event">×</button></div><div class="market-pulse-card-title"><h4>${escapeHtml(event.address)}</h4><p>${escapeHtml(event.suburb)}</p></div><div class="market-pulse-card-content">${details}<div class="market-pulse-card-actions"><button class="${buttonClass}" type="button" data-start-market-session="${escapeHtml(event.id)}" ${matches.length?'':'disabled'}>${matches.length?buttonLabel:'No Matching Contacts'}</button><button class="secondary market-bulk-sms-btn" type="button" data-market-bulk-sms="${escapeHtml(event.id)}" ${bulkSmsReady?'':'disabled'}>Bulk SMS</button></div></div></article>`
...
$('#prospectingView').onclick=async e=>{
  ...
  if(e.target.closest('#broadcastReturnProspector')){const returnToMarket=Boolean(selectedBroadcastContext);closeBroadcastBuilder();setProspectorSection(returnToMarket?'market':'today');renderProspecting();return}
