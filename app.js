...
$('#prospectingView').onsubmit=async e=>{
  ...
  if(e.target.id==='prospectLogForm'){e.preventDefault();const form=e.target,submit=form.querySelector('button[type=submit]');if(submit?.disabled)return;if(submit){submit.disabled=true;submit.textContent='Saving…'}const f=new FormData(form),p=prospectById(activeProspectId),next=f.get('nextFollowUp'),outcome=cleanText(f.get('outcome'),80),fromSession=form.dataset.fromSession==='1',interactionId=prospectId();if(!p){toast('Contact could not be found');if(submit)submit.disabled=false;return}prospectInteractions.push({id:interactionId,prospectId:p.id,date:todayKey(),at:Date.now(),type:'Call',outcome,note:cleanText(f.get('note'),2000),nextFollowUp:validDateKey(next)?next:'',metricsApplied:true});prospects=prospects.map(x=>x.id===p.id?normaliseProspect({...x,temperature:f.get('temperature'),lastContact:todayKey(),nextFollowUp:validDateKey(next)?next:'',stage:outcome==='Appointment booked'?'Appointment Booked':outcome==='Appraisal opportunity'?'Appraisal Opportunity':x.stage,updatedAt:Date.now()}):x);try{await saveProspecting({render:false});await applyProspectingOutcomeMetrics(outcome,interactionId);toast('Contact logged');if(fromSession){prospectSessionIndex++;if(prospectSessionIndex>=prospectSessionIds.length){toast('Prospecting session complete');closeProspectDetail()}else showProspectingSession()}else renderProspectDetail(p.id)}catch(err){console.error('Prospecting log save failed',err);toast('The log was saved locally. Please check sync.');if(fromSession&&prospectSessionIndex<prospectSessionIds.length)showProspectingSession();else renderProspectDetail(p.id)}return}
};
