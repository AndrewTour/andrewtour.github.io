...
const CONTACT_DRAFT_FIELDS=['name','phone','email','address','source','stage','temperature','motivation','sellingTimeframe','tags','nextFollowUp','notes','sellerBedrooms','sellerBathrooms','sellerCars','sellerPropertyType','sellerLandSizeBand','sellerTitleType'];
...
function normaliseContactDraftValues(values={}){
  ...
  return{name:cleanText(values.name,120),phone:cleanText(values.phone,50),email:cleanText(values.email,180),address:cleanText(values.address,240),source:cleanText(values.source,100),stage,temperature,motivation,sellingTimeframe,tags:cleanText(values.tags,300),nextFollowUp:validDateKey(values.nextFollowUp)?values.nextFollowUp:'',notes:cleanText(values.notes,3000),sellerBedrooms:Math.max(0,Math.min(8,Number(values.sellerBedrooms)||0)),sellerBathrooms:Math.max(0,Math.min(6,Number(values.sellerBathrooms)||0)),sellerCars:Math.max(0,Math.min(6,Number(values.sellerCars)||0)),sellerPropertyType:SELLER_PROPERTY_TYPES.includes(values.sellerPropertyType)?values.sellerPropertyType:'',sellerLandSizeBand:SELLER_LAND_SIZE_BANDS.includes(values.sellerLandSizeBand)?values.sellerLandSizeBand:'',sellerTitleType:SELLER_TITLE_TYPES.includes(values.sellerTitleType)?values.sellerTitleType:'',addBuyerBrief:Boolean(values.addBuyerBrief)}
}
function contactDraftMeaningful(draft={}){
  ...
  return textFields.some(field=>Boolean(values[field]))||values.sellerBedrooms||values.sellerBathrooms||values.sellerCars||values.sellerPropertyType||values.sellerLandSizeBand||values.sellerTitleType||values.addBuyerBrief||values.stage!=='New Lead'||values.temperature!=='Cold'||values.motivation!==1
}
...
function handleAppSuspend(){persistOpenContactDraft();if(buyerSession.active){const host=$('#prospectingSession');buyerSession.visible=Boolean(buyerSession.visible&&activeViewId()==='prospectingView'&&host?.dataset.sessionKind==='buyer');saveBuyerSession()}if(pendingProspectingPayload)flushProspectingSave()}
...
$('#prospectingView').addEventListener('change',e=>{
  ...
  form.querySelector('[data-seller-property-wrap]')?.classList.toggle('hidden',!SELLING_TIMEFRAMES.includes(timeframe)&&!sellerPropertyConfigured(Object.fromEntries(new FormData(form))));
...
$('#prospectingView').onsubmit=async e=>{
  ...
  if(e.target.id==='prospectLogForm'){
    ...
    prospectInteractions=prospectInteractions.map(x=>x.id===interactionId?{...x,metricsApplied:true}:x);
    try{await saveProspecting({render:false,awaitCloud:false})}catch(err){console.error('Prospecting log save failed',err);toast('The log was saved locally. Please check sync.')}
