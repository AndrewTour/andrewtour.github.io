...
function openOffDayQuickAppointment(){
  editingAppointment=null;appointmentEditReturnState=null;appointmentHistoryMode=null;appointmentDate=todayKey();appointmentLinkedProspectId='';pendingProspectAppointmentFlow=null;$('#appointmentForm')?.reset();$('#appointmentDatePicker').value=appointmentDate;$('#appointmentTime').value='12:00';$('#appointmentAuction').checked=false;updateOfiFormState();switchView('appointmentsView');requestAnimationFrame(()=>$('#appointmentContactName')?.focus({preventScroll:true}))
}
function openOffDayContactSearch(){setProspectorSection('contacts');switchView('prospectingView');requestAnimationFrame(()=>$('#prospectSearch')?.focus({preventScroll:true}))}
function openOffDayBroadcast(){selectedBroadcastType='';setProspectorSection('broadcast');switchView('prospectingView')}
function runOffDayQuickAction(action=''){
  if(action==='call'){openManualDialler();return}if(action==='task'){openTaskComposer();return}if(action==='appointment'){openOffDayQuickAppointment();return}if(action==='search'){openOffDayContactSearch();return}if(action==='broadcast')openOffDayBroadcast()
}
...
function renderLeaderboardPosition(){
  ...
  meta.textContent=`${index+1} of ${rows.length} agent${rows.length===1?'':'s'}`;
}
...
$('#offDayConversations')?.addEventListener('click',event=>{
  const quickAction=event.target.closest('[data-off-day-quick]');if(quickAction){event.preventDefault();runOffDayQuickAction(quickAction.dataset.offDayQuick);return}
