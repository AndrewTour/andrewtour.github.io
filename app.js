...
function subscribeMarketPulseInbox(){
  ...
  unsubMarketPulseInbox=onSnapshot(pendingQuery,{includeMetadataChanges:true},snap=>{if(snap.metadata.fromCache)return;if(snap.docs.length)enqueueMarketPulseInboxDocuments(snap.docs);else if(marketPulseAutomation.state==='processing'){marketPulseAutomation={...marketPulseAutomation,state:marketPulseAutomation.lastImportedAt?'live':'waiting'};renderMarketPulseAutomationSettings()}},err=>{console.error('MarketPulse inbox sync failed',err);marketPulseAutomation={...marketPulseAutomation,state:'error',error:'Automatic MarketPulse intake could not connect.'};renderMarketPulseAutomationSettings()});
}
...
async function startCloud(user,{promptTeamSetup=false}={}){
  ...
  unsubProfile=onSnapshot(doc(db,'users',uid),{includeMetadataChanges:true},snap=>{
    ...
    applyMarketPulseAutomationProfile(profile);registerMarketPulseForwardingIdentity(profile).catch(err=>console.error('MarketPulse identity setup failed',err));
