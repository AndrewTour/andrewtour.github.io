...
let pendingLocalSaveKinds=new Set(),localSaveTimer=0,localIdleHandle=0;
function writeLocalValue(key,value){const next=typeof value==='string'?value:JSON.stringify(value);if(localStorage.getItem(key)===next)return false;localStorage.setItem(key,next);return true}
function saveDaysLocal(){const prefix=storagePrefix(uid);try{const serialised=JSON.stringify(normaliseDaysMap(days)),key=prefix+'days',previous=localStorage.getItem(key);if(previous===serialised)return true;if(previous)localStorage.setItem(prefix+'days-backup',previous);localStorage.setItem(key,serialised);return true}catch(err){console.error('Day cache save failed',err);return false}}
function saveProfileLocal(){const prefix=storagePrefix(uid);try{writeLocalValue(prefix+'targets',targets);writeLocalValue(prefix+'agent-name',agentName);writeLocalValue(prefix+'work-days',workDays);writeLocalValue(prefix+'calendar-preference',calendarPreference);writeLocalValue(prefix+'local-schema',String(LOCAL_SCHEMA_VERSION));return true}catch(err){console.error('Profile cache save failed',err);return false}}
function saveProspectingLocal(){const prefix=storagePrefix(uid);try{writeLocalValue(prefix+'prospects',prospects);writeLocalValue(prefix+'prospect-interactions',prospectInteractions);writeLocalValue(prefix+'local-schema',String(LOCAL_SCHEMA_VERSION));return true}catch(err){console.error('Prospector cache save failed',err);return false}}
function flushLocalSaves(){clearTimeout(localSaveTimer);localSaveTimer=0;if(localIdleHandle&&window.cancelIdleCallback)cancelIdleCallback(localIdleHandle);localIdleHandle=0;const kinds=[...pendingLocalSaveKinds];pendingLocalSaveKinds.clear();for(const kind of kinds){if(kind==='days')saveDaysLocal();else if(kind==='profile')saveProfileLocal();else if(kind==='prospecting')saveProspectingLocal()}}
function queueLocalSave(kind){pendingLocalSaveKinds.add(kind);if(localSaveTimer||localIdleHandle)return;const run=()=>{localIdleHandle=0;flushLocalSaves()};if(window.requestIdleCallback)localIdleHandle=requestIdleCallback(run,{timeout:120});else localSaveTimer=setTimeout(run,48)}
function saveLocal(){return saveDaysLocal()&&saveProfileLocal()&&saveProspectingLocal()}
...
async function saveTargets(){saveProfileLocal();if(!cloud)return;beginSyncOperation();try{await setDoc(doc(db,'users',uid),{targets,workDays:[...workDays],name:displayAgentName(),email:currentUser?.email||'',updatedAt:serverTimestamp()},{merge:true});scheduleLeaderboardPublish();endSyncOperation()}catch(err){console.error(err);endSyncOperation({error:true});toast('Targets saved locally. Cloud sync failed.')}}
...
function normaliseProspect(raw={}){
  ...
  return{id:cleanText(p.id,80)||prospectId(),name:cleanText(p.name,120)||'Unnamed contact',phone:cleanText(p.phone,50),email:cleanText(p.email,180),address:cleanText(p.address,240),company:cleanText(p.company,240),suburb:cleanText(p.suburb,100),tags:Array.isArray(p.tags)?p.tags.map(x=>cleanText(x,50)).filter(Boolean).slice(0,12):cleanText(p.tags,300).split(/[,;|]/).map(x=>x.trim()).filter(Boolean).slice(0,12),source:cleanText(p.source,100),stage:cleanText(p.stage,60)||'Nurture',temperature:['Cold','Warm','Hot'].includes(p.temperature)?p.temperature:'Cold',motivation:Math.max(1,Math.min(5,Number(p.motivation)||2)),lastContact:validDateKey(p.lastContact)?p.lastContact:'',nextFollowUp:validDateKey(p.nextFollowUp)?p.nextFollowUp:'',notes:cleanText(p.notes,3000),createdAt:Number(p.createdAt)||Date.now(),updatedAt:Number(p.updatedAt)||Date.now()}
}
...
function setProspectorSection(section='today'){
  prospectSection=section==='contacts'?'contacts':'today';
  ...
  const input=$('#prospectSearch');if(input)input.placeholder=prospectSection==='contacts'?'Search anything':'Search name, address or phone';
...
function startProspectingSession(){if(prospectSessionActive){showProspectingSession();return}const due=priorityProspects().filter(p=>p.nextFollowUp&&p.nextFollowUp<=todayKey());prospectSessionIds=(due.length?due:priorityProspects()).slice(0,50).map(p=>p.id);prospectSessionIndex=0;prospectSessionActive=true;prospectSessionStats={calls:0,connects:0,temperate:0,appointments:0};if(!prospectSessionIds.length){prospectSessionActive=false;return toast('Add a prospect to begin')}saveProspectingSessionState();showProspectingSession()}
...
async function startCloud(user){
  ...
  unsubDays=onSnapshot(collection(db,'users',uid,'days'),{includeMetadataChanges:true},snap=>{
    ...
    if(dataChanged){saveDaysLocal();renderAll();ensureTick()}else saveDirtyDays();
...
$('#prospectingView').onsubmit=async e=>{
  ...
  if(e.target.id==='prospectLogForm'){e.preventDefault();const form=e.target,submit=form.querySelector('button[type=submit]');if(submit?.disabled)return;if(submit){submit.disabled=true;submit.textContent='Saving…'}const f=new FormData(form),p=prospectById(activeProspectId),next=f.get('nextFollowUp'),outcome=cleanText(f.get('outcome'),80),fromSession=form.dataset.fromSession==='1',interactionId=prospectId(),temperature=cleanText(f.get('temperature'),20)||'Cold';if(!p){toast('Contact could not be found');if(submit)submit.disabled=false;return}prospectInteractions.push({id:interactionId,prospectId:p.id,date:todayKey(),at:Date.now(),type:'Call',outcome,note:cleanText(f.get('note'),2000),nextFollowUp:validDateKey(next)?next:'',metricsApplied:false});prospects=prospects.map(x=>x.id===p.id?normaliseProspect({...x,temperature,lastContact:todayKey(),nextFollowUp:validDateKey(next)?next:'',stage:outcome==='Appointment booked'?'Appointment Booked':outcome==='Appraisal opportunity'?'Appraisal Opportunity':x.stage,updatedAt:Date.now()}):x);
...
window.addEventListener('pagehide',flushLocalSaves);document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden')flushLocalSaves()});
