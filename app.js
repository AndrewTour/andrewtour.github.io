...
let targets={...DEFAULTS}, days={}, prospects=[], prospectInteractions=[], prospectFilter='priority', activeProspectId=null, prospectSessionIds=[], prospectSessionIndex=0, selectedDate=dateKey(new Date()), appointmentDate=selectedDate, appointmentHistoryMode=null, agentName='', calendarPreference='outlook', leaderboardEntries=[], leaderboardMode='day', leaderboardDayOffset=0, leaderboardWeekOffset=0, scorecardWeekOffset=0;
...
function normaliseProspect(raw={}){
  ...
  return{id:cleanText(p.id,80)||prospectId(),name:cleanText(p.name,120)||'Unnamed contact',phone:cleanText(p.phone,50),email:cleanText(p.email,180),address:cleanText(p.address,240),suburb:cleanText(p.suburb,100),tags:Array.isArray(p.tags)?p.tags.map(x=>cleanText(x,50)).filter(Boolean).slice(0,12):cleanText(p.tags,300).split(/[,;|]/).map(x=>x.trim()).filter(Boolean).slice(0,12),source:cleanText(p.source,100),stage:cleanText(p.stage,60)||'Nurture',temperature:['Cold','Warm','Hot'].includes(p.temperature)?p.temperature:'Warm',motivation:Math.max(1,Math.min(5,Number(p.motivation)||2)),lastContact:validDateKey(p.lastContact)?p.lastContact:'',nextFollowUp:validDateKey(p.nextFollowUp)?p.nextFollowUp:'',notes:cleanText(p.notes,3000),createdAt:Number(p.createdAt)||Date.now(),updatedAt:Number(p.updatedAt)||Date.now()}
}
...
function filteredProspects(){const q=cleanText($('#prospectSearch')?.value||'',120).toLowerCase();let list=priorityProspects();if(prospectFilter==='overdue')list=list.filter(p=>p.nextFollowUp&&p.nextFollowUp<todayKey());else if(prospectFilter==='today')list=list.filter(p=>p.nextFollowUp===todayKey());else if(prospectFilter==='hot')list=list.filter(p=>p.temperature==='Hot');else if(prospectFilter==='appraisal')list=list.filter(p=>prospectorStage(p)==='Appraisal');if(q)list=list.filter(p=>[p.name,p.phone,p.email,p.address,p.suburb,p.source,p.stage,...p.tags].join(' ').toLowerCase().includes(q));return list}
...
function prospectCard(p){const initials=p.name.split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase();return`<button class="prospect-card" type="button" data-open-prospect="${p.id}"><span class="prospect-avatar">${escapeHtml(initials||'P')}</span><span class="prospect-card-copy"><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(p.address||p.phone||p.email||'Contact details not added')}</small><em class="${p.nextFollowUp&&p.nextFollowUp<=todayKey()?'due':''}">${escapeHtml(dueText(p))}</em></span><span class="prospect-temp temp-${p.temperature.toLowerCase()}">${p.temperature}</span><b aria-hidden="true">›</b></button>`}
function renderProspecting(){
  if(!$('#prospectingView'))return;
  const today=todayKey(),overdue=prospects.filter(p=>p.nextFollowUp&&p.nextFollowUp<today).length,due=prospects.filter(p=>p.nextFollowUp===today).length,hot=prospects.filter(p=>p.temperature==='Hot').length,priority=priorityProspects().filter(p=>p.nextFollowUp&&p.nextFollowUp<=today);
  $('#prospectingOverdue').textContent=overdue;$('#prospectingToday').textContent=due;$('#prospectingHot').textContent=hot;$('#prospectingDueCount').textContent=priority.length;$('#prospectingDueLabel').textContent=priority.length===1?'follow-up ready':'follow-ups ready';
  const queue=priorityProspects().filter(p=>p.nextFollowUp&&p.nextFollowUp<=today).slice(0,5);$('#prospectQueue').innerHTML=queue.length?queue.map(prospectCard).join(''):'<div class="prospect-empty"><strong>Queue cleared</strong><small>No overdue follow-ups. Your next opportunities are below.</small></div>';
  const list=filteredProspects();$('#prospectContactList').innerHTML=list.length?list.slice(0,200).map(prospectCard).join(''):'<div class="prospect-empty"><strong>No contacts found</strong><small>Add a prospect or import a CSV to begin.</small></div>';
  if(activeProspectId&&!$('#prospectDetail').classList.contains('hidden'))renderProspectDetail(activeProspectId);
}
function prospectForm(p={}){return`<form id="prospectEditor" class="prospect-editor glass"><div class="prospect-detail-nav"><button type="button" data-close-prospect>‹ Back</button><strong>${p.id?'Edit Contact':'New Contact'}</strong><span></span></div><label>Name<input name="name" value="${escapeHtml(p.name||'')}" required></label><div class="prospect-form-grid"><label>Phone<input name="phone" type="tel" value="${escapeHtml(p.phone||'')}"></label><label>Email<input name="email" type="email" value="${escapeHtml(p.email||'')}"></label></div><label>Address<input name="address" value="${escapeHtml(p.address||'')}"></label><div class="prospect-form-grid"><label>Source<input name="source" value="${escapeHtml(p.source||'')}" placeholder="Door knock, database…"></label><label>Stage<select name="stage">${['New Lead','Nurture','Appraisal Opportunity','Appointment Booked','Pipeline','Past Client'].map(x=>`<option ${p.stage===x?'selected':''}>${x}</option>`).join('')}</select></label></div><div class="prospect-form-grid"><label>Temperature<select name="temperature">${['Cold','Warm','Hot'].map(x=>`<option ${p.temperature===x?'selected':''}>${x}</option>`).join('')}</select></label><label>Motivation<select name="motivation">${[1,2,3,4,5].map(x=>`<option value="${x}" ${Number(p.motivation)===x?'selected':''}>${x} / 5</option>`).join('')}</select></label></div><label>Tags<input name="tags" value="${escapeHtml((p.tags||[]).join(', '))}" placeholder="Vendor, Toongabbie, Past client"></label><label>Next follow-up<input name="nextFollowUp" type="date" value="${p.nextFollowUp||''}"></label><label>Background notes<textarea name="notes" rows="4" placeholder="Long-term context, plans and personal details">${escapeHtml(p.notes||'')}</textarea></label><button class="primary" type="submit">${p.id?'Save Contact':'Add Contact'}</button></form>`}
...
function openProspectLog(id,fromSession=false){const p=prospectById(id);if(!p)return;$('#prospectDetail').classList.remove('hidden');$('#prospectingDashboard').classList.add('hidden');$('#prospectingSession').classList.add('hidden');activeProspectId=id;$('#prospectDetail').innerHTML=`<form id="prospectLogForm" class="prospect-editor glass" data-from-session="${fromSession?'1':'0'}"><div class="prospect-detail-nav"><button type="button" data-cancel-log>‹ Back</button><strong>Log Contact</strong><span></span></div><div class="prospect-log-person"><span>${escapeHtml(p.name)}</span><small>${escapeHtml(p.phone||p.address||'')}</small></div><label>Outcome<select name="outcome"><option>Connected</option><option>No answer</option><option>Left voicemail</option><option>Sent SMS</option><option>Appraisal opportunity</option><option>Appointment booked</option><option>Not interested</option><option>Do not contact</option></select></label><label>Conversation note<textarea name="note" rows="5" placeholder="What changed? What matters next?"></textarea></label><div class="prospect-form-grid"><label>Temperature<select name="temperature">${['Cold','Warm','Hot'].map(x=>`<option ${p.temperature===x?'selected':''}>${x}</option>`).join('')}</select></label><label>Next follow-up<input name="nextFollowUp" type="date" value="${p.nextFollowUp||''}"></label></div><button class="primary" type="submit">Save & ${fromSession?'Next':'Finish'}</button></form>`}
function startProspectingSession(){const due=priorityProspects().filter(p=>p.nextFollowUp&&p.nextFollowUp<=todayKey());prospectSessionIds=(due.length?due:priorityProspects()).slice(0,50).map(p=>p.id);prospectSessionIndex=0;if(!prospectSessionIds.length)return toast('Add a prospect to begin');showProspectingSession()}
function showProspectingSession(){const id=prospectSessionIds[prospectSessionIndex],p=prospectById(id);if(!p){closeProspectDetail();return}$('#prospectingDashboard').classList.add('hidden');$('#prospectDetail').classList.add('hidden');$('#prospectingSession').classList.remove('hidden');const tel=p.phone?`tel:${p.phone.replace(/[^+\d]/g,'')}`:'#';$('#prospectingSession').innerHTML=`<div class="prospect-session-head"><button type="button" data-end-session>End</button><span>${prospectSessionIndex+1} of ${prospectSessionIds.length}</span></div><section class="prospect-session-card glass"><span class="prospect-avatar session-avatar">${escapeHtml(p.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase())}</span><span>${escapeHtml(p.stage)} · ${p.temperature}</span><h2>${escapeHtml(p.name)}</h2><p>${escapeHtml(p.address||p.phone||'No contact detail added')}</p><div class="prospect-session-context"><div><span>LAST CONTACT</span><strong>${p.lastContact?fmtDate(p.lastContact):'Never'}</strong></div><div><span>NEXT FOLLOW-UP</span><strong>${p.nextFollowUp?fmtDate(p.nextFollowUp):'Not set'}</strong></div></div>${p.notes?`<blockquote>${escapeHtml(p.notes)}</blockquote>`:''}<a class="primary ${p.phone?'':'disabled'}" href="${tel}">Call ${escapeHtml(p.name.split(' ')[0])}</a><button class="secondary" type="button" data-session-log="${p.id}">Log Outcome</button><button class="text-btn" type="button" data-session-skip>Skip for now</button></section>`}
...
async function importProspectCsv(file){const rows=parseCsv(await file.text());if(rows.length<2)throw new Error('No contact rows found');const headers=rows.shift().map(x=>x.trim().toLowerCase());const find=(obj,names)=>{for(const n of names){const key=headers.findIndex(h=>h===n||h.includes(n));if(key>=0&&obj[key])return obj[key]}return''};let added=0;for(const r of rows){const name=find(r,['name','contact name','full name'])||[find(r,['first name']),find(r,['last name'])].filter(Boolean).join(' ');const phone=find(r,['mobile','phone','telephone']);const email=find(r,['email']);const address=find(r,['property address','street address','address']);if(!name&&!phone&&!email&&!address)continue;prospects.push(normaliseProspect({name:name||'Unnamed contact',phone,email,address,suburb:find(r,['suburb']),source:find(r,['source']),tags:find(r,['tags','category']),stage:find(r,['stage'])||'Nurture',temperature:find(r,['temperature'])||'Warm',nextFollowUp:find(r,['next follow up','follow up date'])}));added++}prospects=normaliseProspects(prospects);await saveProspecting();toast(`${added} contact${added===1?'':'s'} imported`)}
...
$('#prospectSearch').oninput=()=>renderProspecting();
...
$('#prospectingView').onclick=e=>{
  ...
  if(e.target.closest('[data-cancel-log]')){renderProspectDetail(activeProspectId);return}
  ...
  if(e.target.closest('[data-end-session]')){closeProspectDetail();return}
  const slog=e.target.closest('[data-session-log]');if(slog){openProspectLog(slog.dataset.sessionLog,true);return}
  if(e.target.closest('[data-session-skip]')){prospectSessionIndex++;if(prospectSessionIndex>=prospectSessionIds.length)toast('Session complete');prospectSessionIndex%=prospectSessionIds.length;showProspectingSession();return}
};
$('#prospectingView').onsubmit=async e=>{
  if(e.target.id==='prospectEditor'){e.preventDefault();const f=new FormData(e.target);await upsertProspect({name:f.get('name'),phone:f.get('phone'),email:f.get('email'),address:f.get('address'),source:f.get('source'),stage:f.get('stage'),temperature:f.get('temperature'),motivation:f.get('motivation'),tags:f.get('tags'),nextFollowUp:f.get('nextFollowUp'),notes:f.get('notes')},activeProspectId||'');toast(activeProspectId?'Contact saved':'Contact added');return}
  if(e.target.id==='prospectLogForm'){e.preventDefault();const f=new FormData(e.target),p=prospectById(activeProspectId),next=f.get('nextFollowUp'),outcome=cleanText(f.get('outcome'),80);prospectInteractions.push({id:prospectId(),prospectId:p.id,date:todayKey(),at:Date.now(),type:'Call',outcome,note:cleanText(f.get('note'),2000),nextFollowUp:validDateKey(next)?next:''});prospects=prospects.map(x=>x.id===p.id?normaliseProspect({...x,temperature:f.get('temperature'),lastContact:todayKey(),nextFollowUp:validDateKey(next)?next:'',stage:outcome==='Appointment booked'?'Appointment Booked':outcome==='Appraisal opportunity'?'Appraisal Opportunity':x.stage,updatedAt:Date.now()}):x);await saveProspecting();const fromSession=e.target.dataset.fromSession==='1';toast('Contact logged');if(fromSession){prospectSessionIndex++;if(prospectSessionIndex>=prospectSessionIds.length){toast('Prospecting session complete');closeProspectDetail()}else showProspectingSession()}else renderProspectDetail(p.id);return}
};
...
// AGNT v2.0 — Prospector presentation and workflow layer.
let prospectorTab='today',prospectorPipelineStage='Nurture',prospectorImportRows=[],prospectorImportHeaders=[];
function prospectorStage(p){const s=(p.stage||'').toLowerCase();if(s.includes('list'))return'Listing';if(s.includes('appraisal')||s.includes('appointment'))return'Appraisal';if(s.includes('likely')||p.temperature==='Hot')return'Likely';if(s.includes('watch')||p.temperature==='Warm')return'Watching';return'Nurture'}
function prospectorDaysSince(key){if(!validDateKey(key))return null;return Math.max(0,Math.floor((new Date(todayKey()+'T12:00:00')-new Date(key+'T12:00:00'))/86400000))}
function prospectorRelationshipLines(p){const lines=[];const days=prospectorDaysSince(p.lastContact);lines.push(days===null?'No conversation logged yet.':days===0?'Spoke today.':`Spoke ${days} day${days===1?'':'s'} ago.`);if(p.notes)lines.push(...p.notes.split(/[\n.!?]+/).map(x=>x.trim()).filter(Boolean).slice(0,3));else{if(p.stage)lines.push(`${p.stage} relationship.`);if(p.address)lines.push(`Property: ${p.address}.`)}return lines.slice(0,5)}
function prospectorReason(p){if(p.nextFollowUp&&p.nextFollowUp<todayKey())return`Follow-up overdue since ${fmtDate(p.nextFollowUp)}`;if(p.nextFollowUp===todayKey())return'Follow-up due today';if(p.temperature==='Hot')return'Hot relationship requiring momentum';if(!p.lastContact)return'New relationship to qualify';return'Next best relationship to progress'}
function setProspectorTab(tab){prospectorTab=tab;$$('[data-prospector-tab]').forEach(b=>b.classList.toggle('active',b.dataset.prospectorTab===tab));$$('[data-prospector-panel]').forEach(p=>p.classList.toggle('active',p.dataset.prospectorPanel===tab));if(tab==='contacts')setTimeout(()=>$('#prospectSearch')?.focus(),80);renderProspecting()}
function prospectCard(p){const initials=p.name.split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase();return`<button class="prospect-card prospector-contact-card" type="button" data-open-prospect="${p.id}"><span class="prospect-avatar">${escapeHtml(initials||'P')}</span><span class="prospect-card-copy"><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(p.address||p.phone||p.email||'Property not added')}</small><em class="${p.nextFollowUp&&p.nextFollowUp<=todayKey()?'due':''}">${escapeHtml(dueText(p))}</em></span><span class="prospect-temp temp-${p.temperature.toLowerCase()}">${p.temperature}</span><b aria-hidden="true">›</b></button>`}
function renderProspecting(){
 if(!$('#prospectingView'))return;
 const today=todayKey(),overdue=prospects.filter(p=>p.nextFollowUp&&p.nextFollowUp<today).length,due=prospects.filter(p=>p.nextFollowUp===today).length,hot=prospects.filter(p=>p.temperature==='Hot').length,appraisal=prospects.filter(p=>prospectorStage(p)==='Appraisal').length,focus=overdue+due;
 $('#prospectingOverdue').textContent=overdue;$('#prospectingToday').textContent=due;$('#prospectingHot').textContent=hot;$('#prospectingAppraisal').textContent=appraisal;$('#prospectingDueCount').textContent=focus;$('#prospectingDueLabel').textContent=focus===1?'contact needs attention':'contacts need attention';
 const queue=priorityProspects().slice(0,5);$('#prospectQueue').innerHTML=queue.length?queue.map(prospectCard).join(''):'<div class="prospect-empty"><strong>Your list is clear</strong><small>Import contacts or add a relationship to begin.</small></div>';
 const list=filteredProspects();$('#prospectContactList').innerHTML=list.length?list.slice(0,500).map(prospectCard).join(''):'<div class="prospect-empty"><strong>No contacts found</strong><small>Try another search or import a CSV.</small></div>';
 const pipeline=priorityProspects().filter(p=>prospectorStage(p)===prospectorPipelineStage);$('#prospectorPipelineList').innerHTML=pipeline.length?pipeline.map(prospectCard).join(''):`<div class="prospect-empty"><strong>No ${escapeHtml(prospectorPipelineStage)} contacts</strong><small>Contacts appear here as their relationship progresses.</small></div>`;
 const stale=prospects.filter(p=>{const d=prospectorDaysSince(p.lastContact);return d===null||d>=90}).length,hotNoFollow=prospects.filter(p=>p.temperature==='Hot'&&!p.nextFollowUp).length,missingAddress=prospects.filter(p=>!p.address).length,duplicates=prospects.filter((p,i,a)=>a.findIndex(x=>(p.phone&&x.phone===p.phone)||(p.email&&x.email.toLowerCase()===p.email.toLowerCase()))!==i).length;
 const health=[[stale,'people haven’t been called in 90 days','Reconnect'],[hotNoFollow,'Hot prospects have no follow-up','Schedule'],[missingAddress,'contacts are missing property addresses','Complete'],[duplicates,'possible duplicate contacts','Review'],[appraisal,'appraisal opportunities are active','Progress']];
 $('#prospectorHealthList').innerHTML=health.map(([n,t,a])=>`<button type="button" data-health-action="${a}"><strong>${n}</strong><span>${t}</span><b>${a} ›</b></button>`).join('');
 const completed=prospectInteractions.filter(x=>x.date===today).length,connected=prospectInteractions.filter(x=>x.date===today&&/connected|appraisal|appointment/i.test(x.outcome)).length,opps=prospectInteractions.filter(x=>x.date===today&&/appraisal|appointment/i.test(x.outcome)).length;
 $('#prospectorMomentumHeadline').textContent=completed?`${completed} contact${completed===1?'':'s'} completed.`:'Ready when you are.';$('#prospectorMomentumCopy').textContent=completed?`${connected} quality conversation${connected===1?'':'s'} · ${opps} appraisal opportunit${opps===1?'y':'ies'}. Keep going.`:'Start a session and work the list one relationship at a time.';
 $$('[data-prospector-tab]').forEach(b=>b.classList.toggle('active',b.dataset.prospectorTab===prospectorTab));$$('[data-prospector-panel]').forEach(p=>p.classList.toggle('active',p.dataset.prospectorPanel===prospectorTab));
 if(activeProspectId&&!$('#prospectDetail').classList.contains('hidden'))renderProspectDetail(activeProspectId);
}
function renderProspectDetail(id){const p=prospectById(id);if(!p)return closeProspectDetail();activeProspectId=id;const history=interactionsFor(id),tel=p.phone?`tel:${p.phone.replace(/[^+\d]/g,'')}`:'#',sms=p.phone?`sms:${p.phone.replace(/[^+\d]/g,'')}`:'#',mail=p.email?`mailto:${p.email}`:'#',lines=prospectorRelationshipLines(p);$('#prospectDetail').innerHTML=`<div class="prospect-detail-nav"><button type="button" data-close-prospect>‹ Back</button><strong>Contact</strong><button type="button" data-edit-prospect="${p.id}">Edit</button></div><section class="prospect-profile glass prospector-profile"><div class="prospector-property-label">PROPERTY</div><h2>${escapeHtml(p.address||'Property not added')}</h2><div class="prospector-owner-row"><div><span>OWNER</span><strong>${escapeHtml(p.name)}</strong></div><span class="prospect-temp temp-${p.temperature.toLowerCase()}">${p.temperature}</span></div><div class="prospector-stage-line"><span>${escapeHtml(prospectorStage(p))}</span><small>${escapeHtml(prospectorReason(p))}</small></div><div class="prospect-quick-actions prospector-actions"><a href="${tel}" class="${p.phone?'':'disabled'}">Call</a><a href="${sms}" class="${p.phone?'':'disabled'}">SMS</a><a href="${mail}" class="${p.email?'':'disabled'}">Email</a><button type="button" data-log-prospect="${p.id}">Follow Up</button></div><div class="relationship-snapshot"><span>RELATIONSHIP SNAPSHOT</span>${lines.map(x=>`<p>${escapeHtml(x)}</p>`).join('')}</div></section><section class="prospecting-section glass"><div class="prospecting-section-head"><div><span>TIMELINE</span><h3>Everything together</h3></div></div><div class="prospect-history">${history.length?history.map(x=>`<article><i></i><div><strong>${escapeHtml(x.outcome||x.type)}</strong><small>${fmtDate(x.date)} · ${new Date(x.at).toLocaleTimeString('en-AU',{hour:'numeric',minute:'2-digit'})}</small>${x.note?`<p>${escapeHtml(x.note)}</p>`:''}${x.nextFollowUp?`<em>Follow-up: ${fmtDate(x.nextFollowUp)}</em>`:''}</div></article>`).join(''):'<div class="prospect-empty"><strong>No activity yet</strong><small>Log the first conversation to build the relationship timeline.</small></div>'}</div></section><button class="prospect-delete" type="button" data-delete-prospect="${p.id}">Delete Contact</button>`}
function showProspectingSession(){const id=prospectSessionIds[prospectSessionIndex],p=prospectById(id);if(!p){closeProspectDetail();return}$('#prospectingDashboard').classList.add('hidden');$('#prospectDetail').classList.add('hidden');$('#prospectingSession').classList.remove('hidden');const tel=p.phone?`tel:${p.phone.replace(/[^+\d]/g,'')}`:'#',remember=prospectorRelationshipLines(p).slice(1,4);$('#prospectingSession').innerHTML=`<div class="prospect-session-head"><button type="button" data-end-session>End</button><span>${prospectSessionIndex+1} of ${prospectSessionIds.length}</span></div><section class="prospect-session-card glass prospector-session-v2"><span class="prospector-session-property">PROPERTY</span><h2>${escapeHtml(p.address||'Property not added')}</h2><h3>${escapeHtml(p.name)}</h3><div class="prospector-session-meta"><span class="prospect-temp temp-${p.temperature.toLowerCase()}">${p.temperature}</span><span>${escapeHtml(prospectorStage(p))}</span></div><div class="prospector-call-reason"><span>WHY THEY’RE HERE</span><strong>${escapeHtml(prospectorReason(p))}</strong></div><div class="before-you-call"><span>BEFORE YOU CALL</span>${remember.length?remember.map(x=>`<p>• ${escapeHtml(x)}</p>`).join(''):'<p>• No relationship notes yet.</p>'}</div><a class="primary ${p.phone?'':'disabled'}" href="${tel}">Call ${escapeHtml(p.name.split(' ')[0])}</a><button class="secondary" type="button" data-session-log="${p.id}">Log Outcome</button><button class="text-btn" type="button" data-session-skip>Skip for now</button></section>`}
function openProspectorImportPreview(file){file.text().then(text=>{const rows=parseCsv(text);if(rows.length<2)throw new Error('No rows');prospectorImportHeaders=rows.shift().map(x=>x.trim());prospectorImportRows=rows.slice(0,5000);const sample=prospectorImportRows.slice(0,3);$('#prospectorImportSheet').innerHTML=`<div class="prospector-import-card glass"><div class="prospect-detail-nav"><button type="button" data-close-import>Cancel</button><strong>Import Contacts</strong><span></span></div><span class="prospector-eyebrow">PREVIEW</span><h2>${prospectorImportRows.length} contacts ready</h2><p>AGNT will auto-detect common name, phone, email and property address fields.</p><div class="import-preview-table">${sample.map(r=>`<div>${r.slice(0,4).map(c=>`<span>${escapeHtml(c||'—')}</span>`).join('')}</div>`).join('')}</div><label>Address field<select id="prospectorAddressMode"><option value="auto">Auto Detect</option><option value="address">Address</option><option value="company">Company</option></select></label><button class="primary" type="button" data-confirm-import>Review & Import</button></div>`;$('#prospectorImportSheet').classList.remove('hidden')}).catch(()=>toast('CSV could not be previewed'))}
async function confirmProspectorImport(){const headers=prospectorImportHeaders.map(x=>x.toLowerCase()),rows=[prospectorImportHeaders,...prospectorImportRows].map(r=>r.map(x=>String(x??'')));const csv=rows.map(r=>r.map(v=>`"${v.replace(/"/g,'""')}"`).join(',')).join('\n');await importProspectCsv(new File([csv],'contacts.csv',{type:'text/csv'}));$('#prospectorImportSheet').classList.add('hidden');setProspectorTab('contacts')}
$$('[data-prospector-tab]').forEach(b=>b.addEventListener('click',()=>setProspectorTab(b.dataset.prospectorTab)));
$('#prospectorPipelineSegments')?.addEventListener('click',e=>{const b=e.target.closest('[data-pipeline-stage]');if(!b)return;prospectorPipelineStage=b.dataset.pipelineStage;$$('[data-pipeline-stage]').forEach(x=>x.classList.toggle('active',x===b));renderProspecting()});
$('#prospectCsvImport').onchange=e=>{const file=e.target.files?.[0];if(file)openProspectorImportPreview(file);e.target.value=''};
$('#prospectorImportSheet')?.addEventListener('click',e=>{if(e.target.closest('[data-close-import]'))$('#prospectorImportSheet').classList.add('hidden');if(e.target.closest('[data-confirm-import]'))confirmProspectorImport().catch(err=>{console.error(err);toast('CSV could not be imported')})});
const originalProspectorClick=$('#prospectingView').onclick;$('#prospectingView').onclick=e=>{const tab=e.target.closest('[data-prospector-tab]');if(tab){setProspectorTab(tab.dataset.prospectorTab);return}const stage=e.target.closest('[data-pipeline-stage]');if(stage){prospectorPipelineStage=stage.dataset.pipelineStage;$$('[data-pipeline-stage]').forEach(x=>x.classList.toggle('active',x===stage));renderProspecting();return}const health=e.target.closest('[data-health-action]');if(health){setProspectorTab('contacts');return}originalProspectorClick?.(e)};
