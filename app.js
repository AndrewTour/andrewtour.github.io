...
let targets={...DEFAULTS}, days={}, prospects=[], prospectInteractions=[], prospectFilter='priority', prospectSection='today', activeProspectId=null, prospectSessionIds=[], prospectSessionIndex=0, selectedDate=dateKey(new Date()), appointmentDate=selectedDate, appointmentHistoryMode=null, agentName='', calendarPreference='outlook', leaderboardEntries=[], leaderboardMode='day', leaderboardDayOffset=0, leaderboardWeekOffset=0, scorecardWeekOffset=0;
...
function filteredProspects(){const q=cleanText($('#prospectSearch')?.value||'',120).toLowerCase();let list=priorityProspects();if(prospectFilter==='overdue')list=list.filter(p=>p.nextFollowUp&&p.nextFollowUp<todayKey());else if(prospectFilter==='today')list=list.filter(p=>p.nextFollowUp===todayKey());else if(prospectFilter==='hot')list=list.filter(p=>p.temperature==='Hot');if(q)list=list.filter(p=>[p.name,p.phone,p.email,p.address,p.suburb,p.source,p.stage,...p.tags].join(' ').toLowerCase().includes(q));return list}
...
function prospectCard(p){const initials=p.name.split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase(),activity=prospectActivityClass(p),property=p.address||[p.suburb,p.phone,p.email].filter(Boolean).join(' · ')||'Contact details not added';return`<button class="prospect-card contact-card-v156" type="button" data-open-prospect="${p.id}"><span class="prospect-activity activity-${activity}" aria-hidden="true"></span><span class="prospect-avatar">${escapeHtml(initials||'P')}</span><span class="prospect-card-copy"><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(property)}</small><em class="${p.nextFollowUp&&p.nextFollowUp<=todayKey()?'due':''}">${escapeHtml(dueText(p))}</em></span><span class="prospect-temp temp-${p.temperature.toLowerCase()}">${p.temperature}</span><b aria-hidden="true">›</b></button>`}
function setProspectorSection(section='today'){prospectSection=section==='contacts'?'contacts':'today';$$('[data-prospector-section]').forEach(button=>{const active=button.dataset.prospectorSection===prospectSection;button.classList.toggle('active',active);if(active)button.setAttribute('aria-current','page');else button.removeAttribute('aria-current')});$('#prospectorTodayPanel')?.classList.toggle('hidden',prospectSection!=='today');$('#prospectorContactsPanel')?.classList.toggle('hidden',prospectSection!=='contacts');const input=$('#prospectSearch');if(input)input.placeholder=prospectSection==='contacts'?'Search anything':'Search name, address or phone'}
function renderProspecting(){
  ...
  const list=filteredProspects();$('#prospectContactList').innerHTML=list.length?list.slice(0,200).map(prospectCard).join(''):'<div class="prospect-empty"><strong>No contacts found</strong><small>Try another search, add a prospect or import a CSV.</small></div>';
  const count=$('#prospectContactCount');if(count)count.textContent=`${list.length} contact${list.length===1?'':'s'}`;
...
$('#prospectingView').onclick=e=>{
  const section=e.target.closest('[data-prospector-section]');if(section){setProspectorSection(section.dataset.prospectorSection);return}
