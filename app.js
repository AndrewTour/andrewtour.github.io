...
function dailyBriefingMarketEventMeta(event){const value=event.price||event.guide;if(value)return`${value}${event.guide?' guide':normalisePlace(event.eventType)==='sold'?' sold':''}`;return event.daysOnMarket||event.agency||'Market update'}
...
function dailyBriefingMarketModel(){
  ...
  const rows=events.map(event=>{const matches=marketMatches(event),progress=marketSessionProgress(event,matches),remaining=progress.complete?[]:matches.filter(person=>!progress.workedIds.has(person.id)&&!prospectContactedToday(person.id)),typeKey=normalisePlace(event.eventType),bucket=dailyBriefingEventBucket(event.eventType),preferred=bucket==='listed'||bucket==='sold',priority=hotSpottingPriority(event,remaining.length);return{event,matches,progress,remaining,typeKey,bucket,preferred,priority,active:event.id===activeSessionEventId}}).sort((a,b)=>Number(b.active)-Number(a.active)||Number(b.preferred)-Number(a.preferred)||Number(Boolean(b.remaining.length))-Number(Boolean(a.remaining.length))||b.priority.score-a.priority.score||b.remaining.length-a.remaining.length||b.event.createdAt-a.event.createdAt);
...
function parseMarketPulse(text='',receivedDate=todayKey()){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    events.push({id:marketEventId(eventType,address,suburb,eventDate),eventType,address,suburb,streetKey,receivedDate:eventDate,createdAt:Date.now(),daysOnMarket,propertyDetails,agency,agents,...priceData});
  }
