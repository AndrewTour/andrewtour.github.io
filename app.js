...
function marketSessionProgress(event,matches=marketMatches(event)){
  ...
  const calledIds=new Set(calls.map(x=>x.prospectId)),connectedIds=new Set(calls.filter(x=>PROSPECT_CONNECTED_OUTCOMES.has(x.outcome)).map(x=>x.prospectId)),followUpIds=new Set(calls.filter(x=>validDateKey(x.nextFollowUp)).map(x=>x.prospectId));
  const active=prospectSessionActive&&prospectSessionContext?.eventId===event.id,complete=matches.length>0&&calledIds.size>=matches.length;
  return{total:matches.length,called:Math.min(calledIds.size,matches.length),connects:connectedIds.size,followUps:followUpIds.size,active,complete};
}
