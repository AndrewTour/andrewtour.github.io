...
let unsubDays=null, unsubProfile=null, unsubLeaderboard=null, unsubProspecting=null, unsubMarketPulseInbox=null, unsubTeamMembership=null, unsubTeamMembers=null, unsubAppointmentAssignees=null, unsubAssignedTeamAppointments=null, timerTick=null, syncTimer=null, leaderboardPublishTimer=null, prospectingSaveTimer=null, returningSnapshotTimer=null;
...
function dailyBriefingSeenKey(){return `${storagePrefix(uid)}daily-briefing-seen:${todayKey()}`}
function dailyBriefingStateValue(){return latestMarketPulseEventDate(marketPulseEvents)===todayKey()?`fresh:${todayKey()}`:`awaiting:${todayKey()}`}
function dailyBriefingSeen(){try{return localStorage.getItem(dailyBriefingSeenKey())===dailyBriefingStateValue()}catch{return false}}
function markDailyBriefingSeen(){try{localStorage.setItem(dailyBriefingSeenKey(),dailyBriefingStateValue())}catch{}}
...
function dailyBriefingMarketModel(){
  ...
  const totalIds=new Set(rows.flatMap(row=>row.remaining.map(person=>person.id))),priorityIds=new Set(rows.filter(row=>row.preferred).flatMap(row=>row.remaining.map(person=>person.id))),assigned=new Set(),focusRows=[];
  ...
  return{events,rows,focusRows,activeDate,fresh,hasError,listed,sold,price,other,totalClientCount:totalIds.size,priorityClientCount:priorityIds.size,suburbCount,statusState:hasError?'error':fresh?'fresh':'waiting',statusLabel:hasError?'Needs attention':fresh?'Updated today':'Awaiting today',debrief};
}
...
function dailyBriefingPlan(now,data){
  ...
  if(data.market.fresh&&top){const minutes=estimatedMinutes(top.clientCount);let meta=`${top.event.eventType} · ${top.clientCount} client${top.clientCount===1?'':'s'} · ${formatEstimatedTime(minutes)}.`;if(next&&untilNext>60){const available=Math.max(0,untilNext-20);if(minutes<=available)meta+=` This fits before the ${appointmentTimeLabel(next.entry.appointment,next.entry.sourceDate)} appointment with a 20-minute prep window.`;else if(available>=10){const workable=Math.max(1,Math.min(top.clientCount,Math.floor(available*60/150))),stopAt=timelineTimeLabel(next.minutes-20);meta+=` Work the first ${workable}, then stop by ${stopAt} for appointment preparation.`}}return{title:`Start with ${top.event.address}`,meta,action:'start-market',label:'Start Priority Session',eventId:top.event.id}}
  ...
  if(data.appointments.length){const appointment=data.appointments[0].appointment;return{title:'Prepare the day’s appointments',meta:`${data.appointments.length} appointment${data.appointments.length===1?' is':'s are'} scheduled. Confirm the outcome you need from each conversation.`,action:'view-appointments',label:'View Appointments',eventId:''}}
...
function dailyBriefingModel(now=new Date()){
  const market=dailyBriefingMarketModel(),appointments=appointmentEntriesForDate(todayKey()),nextAppointment=returningSnapshotUpcomingAppointment(now),followUps=allFollowUpsForDate(todayKey()),overdueFollowUps=dailyBriefingOverdueCount(followUps),pipeline=returningSnapshotPipelineCount(),data={market,appointments,nextAppointment,followUps,overdueFollowUps,pipeline};
...
function renderReturningSnapshot(){
  ...
  const now=new Date(),ready=dailyBriefingDataReady(),loading=$('#returningSnapshotLoading'),content=$('#returningSnapshotContent');
  $('#returningSnapshotDate').textContent=now.toLocaleDateString('en-AU',{weekday:'long',day:'numeric',month:'short'});
  $('#returningSnapshotKicker').textContent='YOUR DAILY BRIEF';$('#returningSnapshotGreeting').textContent=welcomeGreetingFor(now);$('#returningSnapshotName').textContent=firstWelcomeName();
  loading?.classList.toggle('hidden',ready);content?.classList.toggle('hidden',!ready);
  if(!ready){$('#returningSnapshotPriority').textContent='Checking MarketPulse, appointments and follow-ups before AGNT sets your first priority.';return}
  const data=dailyBriefingModel(now),market=data.market,next=data.nextAppointment;
  $('#returningSnapshotPriority').textContent=market.fresh?`${market.events.length} market update${market.events.length===1?'':'s'} · ${market.totalClientCount} unworked client${market.totalClientCount===1?'':'s'} matched.`:market.debrief;
  const marketStatus=$('#returningSnapshotMarketStatus');marketStatus.textContent=market.statusLabel;marketStatus.dataset.state=market.statusState;
  $('#returningSnapshotClientCount').textContent=market.totalClientCount;$('#returningSnapshotClientLabel').textContent=market.fresh?(market.totalClientCount===1?'client to speak to':'clients to speak to'):(market.totalClientCount===1?'previous client available':'previous clients available');$('#returningSnapshotMarketTime').textContent=market.totalClientCount?`${formatEstimatedTime(estimatedMinutes(market.totalClientCount))} ${market.fresh?'focused calling':'in previous Hot Spotting'}`:'No matched calling time';
  const counts=[['Listed',market.listed,'listed'],['Sold',market.sold,'sold'],['Price',market.price,'price'],...(market.other?[['Other',market.other,'other']]:[])];$('#returningSnapshotMarketCounts').innerHTML=counts.map(([label,value,state])=>`<span data-state="${state}"><strong>${value}</strong>${label}</span>`).join('');$('#returningSnapshotMarketDebrief').textContent=market.debrief;
  $('#returningSnapshotFocusMeta').textContent=market.fresh?'Listed and Sold ranked first':'Previous opportunities shown for context';
  $('#returningSnapshotFocusList').innerHTML=market.focusRows.length?market.focusRows.map(row=>{const meta=[row.event.suburb,dailyBriefingMarketEventMeta(row.event)].filter(Boolean).join(' · ');return`<button type="button" class="daily-briefing-focus-row" data-briefing-market-event="${escapeHtml(row.event.id)}"><span class="market-event-tag event-${marketTypeClass(row.event.eventType)}">${escapeHtml(row.event.eventType)}</span><span class="daily-briefing-focus-copy"><strong>${escapeHtml(row.event.address)}</strong><small>${escapeHtml(meta)}</small></span><span class="daily-briefing-focus-count"><strong>${row.clientCount}</strong><small>${formatEstimatedTime(estimatedMinutes(row.clientCount))}</small></span><i aria-hidden="true">›</i></button>`}).join(''):`<div class="daily-briefing-focus-empty"><strong>${market.fresh?'No unworked matches':'Waiting for today’s opportunity list'}</strong><small>${market.fresh?'Listed and Sold remain the priority when new matches appear.':'Previous Hot Spotting remains available inside Prospector.'}</small></div>`;
  $('#returningSnapshotAppointments').textContent=data.appointments.length?`${data.appointments.length} today`:'Clear';$('#returningSnapshotAppointmentMeta').textContent=next?`${appointmentTimeLabel(next.entry.appointment,next.entry.sourceDate)} · ${appointmentType(next.entry.appointment)} · ${next.entry.appointment.contactName||next.entry.appointment.name||'Appointment'}`:data.appointments.length?'No appointments remaining':'No appointments today';
  $('#returningSnapshotFollowUps').textContent=data.followUps.length?`${data.followUps.length} due`:'Clear';$('#returningSnapshotFollowUpMeta').textContent=data.overdueFollowUps?`${data.overdueFollowUps} overdue · act first`:data.followUps.length?'Due today':'No follow-ups due';
  $('#returningSnapshotPipeline').textContent=prospectSessionActive&&!cleanText(prospectSessionContext?.eventId,160)?`${data.pipeline} left`:`${data.pipeline} ready`;$('#returningSnapshotPipelineMeta').textContent=prospectSessionActive&&!cleanText(prospectSessionContext?.eventId,160)?'Current session':'Eligible pipeline';
  $('#returningSnapshotNextBlock').textContent=data.plan.title;$('#returningSnapshotNextBlockMeta').textContent=data.plan.meta;
  const primary=$('#returningSnapshotPrimaryAction'),secondary=$('#returningSnapshotSecondaryAction'),singleAction=data.plan.action==='open';primary.textContent=data.plan.label;primary.dataset.briefingAction=data.plan.action;primary.dataset.eventId=data.plan.eventId||'';secondary?.classList.toggle('hidden',singleAction);primary.parentElement?.classList.toggle('single-action',singleAction);
}
function refreshReturningSnapshotIfVisible(){const screen=$('#returningSnapshotScreen');if(!screen||screen.classList.contains('hidden'))return;if(dailyBriefingDataReady()&&returningSnapshotTimer){clearTimeout(returningSnapshotTimer);returningSnapshotTimer=null}renderReturningSnapshot()}
function dismissReturningSnapshot(){
  clearTimeout(returningSnapshotTimer);returningSnapshotTimer=null;const screen=$('#returningSnapshotScreen');if(!screen)return;markDailyBriefingSeen();document.body.classList.remove('daily-briefing-open');$('#app')?.removeAttribute('inert');screen.classList.add('is-leaving');screen.setAttribute('aria-hidden','true');setTimeout(()=>screen.classList.add('hidden'),260);
}
...
function showLaunchExperience(){if(showOffDayReview())return;showReturningSnapshot()}
...
function marketFollowUpTriggerLabel(trigger=''){return{sold:'Sold',price:'Price update',auction:'Auction date',any:'Any update'}[trigger]||''}
...
function mergeParsedMarketPulseEvents(parsed=[]){
  ...
  return{incomingCount:incoming.length,appliedCount:applicable.length,newCount:fresh.length,refreshedCount:applicable.length-fresh.length,replacedCount:Math.max(0,existing.length-activeExisting.length),activeDate,ignoredAsStale:false,closedPreviousSession,triggeredFollowUps};
}
...
function renderMarketPulse(){
  ...
  list.innerHTML=events.length?events.map(event=>{const matches=marketMatches(event),progress=marketSessionProgress(event,matches),remaining=Math.max(0,progress.total-progress.workedIds.size),marketFollowUps=matches.filter(person=>marketTriggeredFollowUp(event,person.id)).length,recency=relativeEventRecency(event),priority=hotSpottingPriority(event,matches.length),buttonLabel=progress.complete?'Session Complete':progress.active?'Active Session':'Start Session',buttonClass=progress.complete?'primary market-session-complete-btn':progress.active?'primary market-session-active-btn':'primary market-session-start-btn';return`<article class="market-pulse-card${progress.complete?' market-session-complete-card':''}" data-market-event-id="${escapeHtml(event.id)}"><div class="market-pulse-card-head"><span class="market-event-tag event-${marketTypeClass(event.eventType)}">${escapeHtml(event.eventType)}</span><button type="button" data-remove-market-event="${escapeHtml(event.id)}" aria-label="Remove event">×</button></div><h4>${escapeHtml(event.address)}</h4><p>${escapeHtml(event.suburb)}</p>${marketEventDetailHtml(event)}${marketFollowUps?`<div class="market-followup-ready"><strong>${marketFollowUps}</strong><span>requested market follow-up${marketFollowUps===1?'':'s'} ready first</span></div>`:''}<div class="hotspot-opportunity-summary"><span><strong>${remaining}</strong> remaining</span><span><strong>${formatEstimatedTime(estimatedMinutes(remaining))}</strong> session</span><span class="priority-${priority.label.toLowerCase()}"><strong>${priority.label}</strong> priority</span><span><strong>${recency.label}</strong> event</span></div><button class="${buttonClass}" type="button" data-start-market-session="${escapeHtml(event.id)}" ${matches.length&&!progress.complete?'':'disabled'}>${matches.length?buttonLabel:'No Matching Contacts'}</button></article>`}).join(''):'<div class="prospect-empty"><strong>No hot spotting opportunities yet</strong><small>Paste today’s email below, then tap Find Opportunities.</small></div>';
}
...
function marketFollowUpFieldMarkup(event,prospectId){
  if(!event?.eventId)return'';const propertyKey=event.propertyKey||marketPropertyKey(event.address,event.suburb),existing=prospectInteractions.find(item=>item.prospectId===prospectId&&item.marketPropertyKey===propertyKey&&item.marketFollowUpStatus==='pending'),selected=existing?.marketFollowUpTrigger||marketFollowUpDefaultTrigger(event);
  const options=[['','Don’t bring back'],['sold','When sold'],['price','On a price update'],['auction','When an auction date appears'],['any','On any new update']];
  return`<section class="market-followup-field" data-market-followup-field><div><span>BRING BACK WHEN</span><small>AGNT will watch this property and return this client once.</small></div><select name="marketFollowUpTrigger">${options.map(([value,label])=>`<option value="${value}" ${selected===value?'selected':''}>${label}</option>`).join('')}</select>${event.auctionDate?`<em>${escapeHtml(marketAuctionLabel(event))} is already available and will be included in future prompts.</em>`:''}</section>`
}
...
function marketFollowUpHistoryMarkup(interaction){
  if(interaction.marketFollowUpStatus==='pending')return`<em class="market-followup-history">Bring back: ${escapeHtml(marketFollowUpTriggerLabel(interaction.marketFollowUpTrigger))} · ${escapeHtml([interaction.marketFollowUpAddress,interaction.marketFollowUpSuburb].filter(Boolean).join(', '))}</em>`;
  if(interaction.marketFollowUpStatus==='triggered')return`<em class="market-followup-history triggered">Brought back: ${escapeHtml(interaction.marketFollowUpTriggeredReason||marketFollowUpTriggerLabel(interaction.marketFollowUpTrigger))}</em>`;return''
}
function marketFollowUpSessionPromptMarkup(event,prospectId){const prompt=marketFollowUpPrompt(event,prospectId);return prompt?`<aside class="market-followup-prompt"><span>MARKET FOLLOW-UP</span><strong>${escapeHtml(prompt.reason)}</strong><p>${escapeHtml(prompt.context)}</p></aside>`:''}
function openProspectLog(id,fromSession=false){
...
function showProspectingSession(){if(!prospectSessionActive)return closeProspectDetail();if(prospectSection!=='today')return;const sessionHost=$('#prospectingSession');if(sessionHost)sessionHost.dataset.sessionView='1';$('#prospectingDashboard').classList.add('hidden');$('#prospectDetail').classList.add('hidden');sessionHost?.classList.remove('hidden');while(prospectSessionIndex<prospectSessionIds.length){const current=prospectById(prospectSessionIds[prospectSessionIndex]);if(current&&!current.archived)break;prospectSessionIndex++}saveProspectingSessionState();const remaining=Math.max(0,prospectSessionIds.length-prospectSessionIndex);if(prospectSessionIndex>=prospectSessionIds.length){$('#prospectingSession').innerHTML=`<div class="prospect-session-head"><button type="button" data-session-back aria-label="Back">‹</button>${prospectSessionHeaderStatus(0)}<button type="button" data-end-session>End Session</button></div><section class="prospect-session-card glass prospect-session-complete"><span class="prospect-avatar session-avatar">✓</span><h2>Queue complete</h2><p>You’ve worked through every contact in this session.</p><button class="primary" type="button" data-complete-market-session>Review & End Session</button>${!cleanText(prospectSessionContext?.eventId,160)?'<button class="prospect-session-refresh-bottom" type="button" data-refresh-pipeline-session><span aria-hidden="true">↻</span> Refresh Session</button>':''}</section>`;return}const id=prospectSessionIds[prospectSessionIndex],p=prospectById(id);const phone=primaryProspectPhone(p),tel=prospectTel(p),marketEvent=prospectSessionContext?.eventId?(marketPulseEvents.find(event=>event.id===prospectSessionContext.eventId)||prospectSessionContext):null;$('#prospectingSession').innerHTML=`<div class="prospect-session-head"><button type="button" data-session-back aria-label="Back">‹</button>${prospectSessionHeaderStatus(remaining)}<button type="button" data-end-session>End Session</button></div>${prospectSessionContextStrip()}<section class="prospect-session-card glass"><span class="prospect-avatar session-avatar">${escapeHtml(p.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase())}</span><span>${escapeHtml(p.stage)} · ${p.temperature}</span><h2>${escapeHtml(p.name)}</h2><p>${escapeHtml(formatProspectAddress(p.address||p.company,p.suburb)||p.phone||'No contact detail added')}</p>${marketEvent?marketFollowUpSessionPromptMarkup(marketEvent,p.id):''}<div class="prospect-session-context"><div><span>LAST CONTACT</span><strong>${p.lastContact?fmtDate(p.lastContact):'Never'}</strong></div><div><span>NEXT FOLLOW-UP</span><strong>${p.nextFollowUp?fmtDate(p.nextFollowUp):'Not set'}</strong></div></div>${p.notes?`<blockquote>${escapeHtml(p.notes)}</blockquote>`:''}<div class="prospect-session-actions"><a class="primary ${phone?'':'disabled'}" href="${tel}" data-prospect-call="${p.id}" data-call-from-session="1">Call ${escapeHtml(p.name.split(' ')[0])}</a><button class="primary hotspot-sms-btn ${phone?'':'disabled'}" type="button" data-session-sms="${p.id}" ${phone?'':'disabled'}>SMS ${escapeHtml(hotSpotSmsFirstName(p))}</button></div><button class="secondary" type="button" data-session-log="${p.id}">Log Outcome</button><button class="text-btn" type="button" data-session-skip>Skip for now</button>${!cleanText(prospectSessionContext?.eventId,160)?'<button class="prospect-session-refresh-bottom" type="button" data-refresh-pipeline-session><span aria-hidden="true">↻</span> Refresh Session</button>':''}</section>`}
...
$('#returningSnapshotScreen')?.addEventListener('click',event=>{
  if(event.target.closest('#returningSnapshotClose,#returningSnapshotSecondaryAction')){dismissReturningSnapshot();return}
  const primary=event.target.closest('#returningSnapshotPrimaryAction');if(primary){openDailyBriefingDestination(primary.dataset.briefingAction,primary.dataset.eventId);return}
  const marketEvent=event.target.closest('[data-briefing-market-event]');if(marketEvent){openDailyBriefingDestination('view-market',marketEvent.dataset.briefingMarketEvent);return}
  const destination=event.target.closest('[data-briefing-view]');if(!destination)return;
  const actions={appointments:'view-appointments',followups:'view-followups',pipeline:'view-pipeline'};openDailyBriefingDestination(actions[destination.dataset.briefingView]||'open');
});
