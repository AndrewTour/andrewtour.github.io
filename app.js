...
function dailyPlanProspectFollowUpComplete(prospect,viewDate){
  if(prospect.recordType==='buyer')return buyerContactedOnDate(prospect.id,viewDate)||prospectInteractions.some(item=>item.prospectId===prospect.id&&item.type==='Follow-up'&&item.outcome==='Follow-up completed'&&item.date===viewDate);
...
function dailyPlanBuyerMatchWorkload(viewDate){
  if(viewDate!==todayKey())return null;const envelopes=buyerMatchContactEnvelopes(viewDate);if(!envelopes.length)return null;const buyerCount=envelopes.length,propertyKeys=new Set(envelopes.flatMap(row=>row.matches.map(match=>match.propertyKey))),propertyCount=propertyKeys.size,duration=dailyPlanRoundMinutes(buyerCount*5,15,45),matches=envelopes.map(row=>({buyerId:row.buyer.id,buyerName:row.buyer.name,matchId:row.primary.id,address:row.primary.address,suburb:row.primary.suburb,eventType:row.primary.eventType,reason:row.primary.reason,phone:row.phone,propertyCount:row.propertyCount,additionalAddresses:row.matches.slice(1).map(match=>[match.address,match.suburb].filter(Boolean).join(', ')),followUpDue:row.followUpDue,followUpLabel:row.followUpLabel,state:buyerMatchStateLabel(row.primary),urgency:row.urgency,sellerOpportunity:buyerSellerOpportunityFor(row.buyer,row.primary)})),sellerCount=matches.filter(match=>match.sellerOpportunity).length,sellerMeta=sellerCount?` · ${sellerCount} seller angle${sellerCount===1?'':'s'}`:'',urgentCount=matches.filter(match=>match.urgency).length,criticalCount=matches.filter(match=>match.urgency?.level==='critical').length,urgentMeta=urgentCount?` · ${urgentCount} time-sensitive`:'';
  return{id:'buyer-matches',workloadId:'buyer-matches',kind:'buyer-match',title:urgentCount?`${urgentCount} time-sensitive buyer conversation${urgentCount===1?'':'s'}`:`${buyerCount} buyer conversation${buyerCount===1?'':'s'} from fresh stock`,meta:`${propertyCount} matching propert${propertyCount===1?'y':'ies'}${urgentMeta}${sellerMeta} · ${duration} min protected`,kicker:urgentCount?`BUYER ALERTS · ${urgentCount} TIME-SENSITIVE`:`BUYER OPPORTUNITIES · ${buyerCount} READY`,duration,minimumSegment:15,clientCount:buyerCount,propertyCount,sellerCount,urgentCount,criticalCount,remainingClients:buyerCount,matches,completed:false,actionable:true,action:'view-buyer-matches',label:'Open buyers',commandTitle:urgentCount?`Contact ${urgentCount} time-sensitive buyer${urgentCount===1?'':'s'}`:`Connect ${buyerCount} buyer${buyerCount===1?'':'s'} to fresh stock`,commandMeta:`${propertyCount} MarketPulse propert${propertyCount===1?'y fits':'ies fit'} their verified brief${buyerCount===1?'':'s'}${urgentCount?` · ${matches.find(match=>match.urgency)?.urgency.label}`:''}${sellerCount?` · ${sellerCount} conversation${sellerCount===1?' also links':'s also link'} a matched home to a current property`:''} · one conversation per buyer · ${duration} protected minutes.`,rank:criticalCount?-85:urgentCount?-70:-60};
}
function dailyPlanFollowUpWorkload(viewDate){
  const buyersWithMatches=new Set(activeBuyerProspects().filter(buyer=>buyerOpenPropertyMatches(buyer).length).map(buyer=>buyer.id)),appointmentFollowUps=scheduledFollowUpsForDate(viewDate),prospectFollowUps=prospectFollowUpsForDate(viewDate).filter(prospect=>!buyersWithMatches.has(prospect.id)&&!dailyPlanProspectFollowUpComplete(prospect,viewDate)),count=appointmentFollowUps.length+prospectFollowUps.length;if(!count)return null;
...
function dailyPlanAssignBuyerMatches(items=[]){
  if(!items.length)return items;const remaining=[...(items[0].matches||[])];items.forEach((item,index)=>{const capacity=Math.max(1,Math.floor((Number(item.duration)||15)/5)),matches=remaining.splice(0,capacity),buyerCount=matches.length,propertyCount=matches.reduce((sum,match)=>sum+Math.max(1,Number(match.propertyCount)||1),0),sellerCount=matches.filter(match=>match.sellerOpportunity).length,sellerMeta=sellerCount?` · ${sellerCount} seller angle${sellerCount===1?'':'s'}`:'',urgentCount=matches.filter(match=>match.urgency).length,urgentMeta=urgentCount?` · ${urgentCount} time-sensitive`:'',overflow=index===items.length-1&&remaining.length?` · ${remaining.length} more in Buyers`:'';item.matches=matches;item.clientCount=buyerCount;item.propertyCount=propertyCount;item.sellerCount=sellerCount;item.urgentCount=urgentCount;item.title=urgentCount?`${urgentCount} time-sensitive buyer conversation${urgentCount===1?'':'s'}`:`${buyerCount} buyer conversation${buyerCount===1?'':'s'}`;item.kicker=urgentCount?`BUYER ALERTS · ${urgentCount} TIME-SENSITIVE`:`BUYER OPPORTUNITIES · ${buyerCount} READY`;item.meta=`${propertyCount} matching propert${propertyCount===1?'y':'ies'}${urgentMeta}${sellerMeta} · ${item.duration} min protected${overflow}`});return items
}
...
function buyerMatchTimeAlert(buyer={},match={},referenceDate=todayKey()){
  if(!buyer||buyer.recordType!=='buyer'||buyer.archived||buyer.buyerStage==='Purchased'||!buyerMatchStatusOpen(match.status))return null;
...
function buyerLinkedSellerProspect(buyer={}){
  ...
  return prospects.find(prospect=>{
    if(!prospect||prospect.id===buyer.id||prospect.recordType==='buyer'||prospect.archived)return false;
...
function buyerSellerOpportunityFor(buyer={},match={}){
  if(!buyer||buyer.recordType!=='buyer'||buyer.archived||buyer.buyerStage==='Purchased'||!match?.propertyKey)return null;
...
function buyerMatchSelected(buyerId,matchId){const buyer=prospectById(buyerId),match=normaliseBuyerPropertyMatches(buyer?.buyerPropertyMatches).find(item=>item.id===matchId);return buyer&&buyer.recordType==='buyer'&&match?{buyer,match}:null}
...
function resumeBuyerMatchSmsReturn(){const pending=loadBuyerMatchSmsPending();if(!pending)return;const age=Date.now()-(Number(pending.openedAt)||0);if(age<400||Date.now()<buyerMatchSmsReturnGuardUntil)return;if(age>10*60*1000){saveBuyerMatchSmsPending(null);return}showBuyerMatchSmsConfirmation(pending)}
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden)setTimeout(resumeBuyerMatchSmsReturn,420)});window.addEventListener('pageshow',()=>setTimeout(resumeBuyerMatchSmsReturn,420));window.addEventListener('focus',()=>setTimeout(resumeBuyerMatchSmsReturn,420));
