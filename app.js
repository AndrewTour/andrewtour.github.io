...
function marketBriefingRowData(row={}){
  const event=row.event||{},kind=marketPulseEventKind(event),category=marketPulseEventPropertyCategory(event),configuration=marketPulseEventConfigurationLabel(event,category),movement=marketMovementLabel(event),auction=marketAuctionLabel(event),price=event.guide?`Guide ${event.guide}`:event.price?(kind==='sold'?`Sold ${event.price}`:`Price ${event.price}`):'',prior=event.priorPrice?`Prior ${event.guide?'guide':'price'} ${event.priorPrice}`:'',timing=relativeEventRecency(event).label,opportunity=row.remaining?.length?`${row.remaining.length} client${row.remaining.length===1?'':'s'} to speak to`:row.matches?.length?`${row.matches.length} matched · worked`:'No matched contacts';
  return{event,kind,label:kind==='listed'?'LISTED':kind==='sold'?'SOLD':kind==='price'?'PRICE UPDATE':String(event.eventType||'MARKETPULSE').toUpperCase(),address:event.address||'Address not recorded',suburb:event.suburb||'',configuration,details:[price,movement,prior,auction,event.daysOnMarket].filter(Boolean),timing,opportunity,requestedFollowUps:Number(row.requestedFollowUps)||0}
}
...
function navigateDailyPlanAction(action='open',eventId=''){
  ...
  if(action==='view-market'){marketReviewFilter='all';marketPageMode='marketpulse';switchView('prospectingView');setProspectorSection('market');renderProspecting();return}
...
function followUpContextCardMarkup(row={}){
  const due=row.overdue?`Overdue · ${fmtDate(row.dueDate)}`:row.dueDate===todayKey()?'Due today':row.dueDate?`Due ${fmtDate(row.dueDate)}`:'Action due',lastDate=row.lastDate?` · ${fmtDate(row.lastDate)}`:'',call=row.dial?`<a class="primary" href="tel:${escapeHtml(row.dial)}" ${row.type==='appointment'?`data-appointment-followup-call="${escapeHtml(row.recordId)}" data-source-date="${escapeHtml(row.sourceDate)}"`:`data-prospect-call="${escapeHtml(row.recordId)}" data-call-from-session="0" data-call-return-mode="followups"`}>Call</a>`:'<button class="primary" type="button" disabled>No phone</button>',action=row.type==='appointment'?`<button class="secondary" type="button" data-context-appointment-outcome="${escapeHtml(row.recordId)}" data-source-date="${escapeHtml(row.sourceDate)}">Update outcome</button>`:`<button class="secondary" type="button" data-context-followup-log="${escapeHtml(row.recordId)}">Log outcome</button>`;
  return`<article class="followup-context-card ${row.overdue?'is-overdue':''}"><div class="followup-context-head"><div><span>${escapeHtml(row.role.toUpperCase())}</span><h3>${escapeHtml(row.name)}</h3></div><b>${escapeHtml(due)}</b></div><div class="followup-context-body"><div><span>WHY IT IS DUE</span><p>${escapeHtml(row.why)}</p></div><div><span>LAST CONTEXT</span><p>${escapeHtml(row.last)}${escapeHtml(lastDate)}</p></div></div><div class="followup-context-actions">${call}${action}</div></article>`
}
...
function openMarketPulseDataArea(){
  marketReviewFilter='all';marketPageMode='marketpulse';switchView('prospectingView');setProspectorSection('market');setMarketPageMode('marketpulse');renderMarketPulse();requestAnimationFrame(()=>{const view=$('#prospectingView');if(view)view.scrollTop=0;window.scrollTo({top:0,behavior:'instant'})});
}
function openHotSpottingArea(){
  marketPageMode='hotspotting';setProspectorSection('market');setMarketPageMode('hotspotting');renderProspecting();requestAnimationFrame(()=>{const view=$('#prospectingView');if(view)view.scrollTop=0});
}
...
$('#openMarketPulseHome')&&($('#openMarketPulseHome').onclick=openMarketPulseDataArea);
...
$('#openMarketPulseData')&&($('#openMarketPulseData').onclick=openMarketPulseDataArea);
$('#backFromMarketPulseData')&&($('#backFromMarketPulseData').onclick=openHotSpottingArea);
