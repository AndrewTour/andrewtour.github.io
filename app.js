...
function switchView(id){if(!document.getElementById(id)?.classList.contains('view'))id='todayView';if(id!=='appointmentsView'&&appointmentHistoryMode){appointmentQuickReturnHome=false;setAppointmentHistoryScreen(null)}$$('.tabbar button').forEach(b=>b.classList.toggle('active',b.dataset.view===id));$$('.view').forEach(v=>v.classList.toggle('active',v.id===id));updateTopbar(id);updateBackTodayVisibility(id);if(id==='todayView')renderToday();if(id==='scheduleView'){renderTimeline();setTodayPage(todayPage);}if(id==='appointmentsView')renderAppointments();if(id==='prospectingView')renderProspecting();if(id==='insightsView')renderInsights();if(id==='settingsView')renderSettings()}
