...
function normaliseProspect(raw={}){
  ...
  return{id:cleanText(p.id,80)||prospectId(),name:cleanText(p.name,120)||'Unnamed contact',phone:cleanText(p.phone,50),email:cleanText(p.email,180),address:cleanText(p.address,240),company:cleanText(p.company,240),suburb:cleanText(p.suburb,100),tags:Array.isArray(p.tags)?p.tags.map(x=>cleanText(x,50)).filter(Boolean).slice(0,12):cleanText(p.tags,300).split(/[,;|]/).map(x=>x.trim()).filter(Boolean).slice(0,12),source:cleanText(p.source,100),stage:cleanText(p.stage,60)||'Nurture',temperature:['Cold','Warm','Hot'].includes(p.temperature)?p.temperature:'Cold',sellingTimeframe:SELLING_TIMEFRAMES.includes(p.sellingTimeframe)?p.sellingTimeframe:'',motivation:Math.max(1,Math.min(5,Number(p.motivation)||2)),lastContact:validDateKey(p.lastContact)?p.lastContact:'',nextFollowUp:validDateKey(p.nextFollowUp)?p.nextFollowUp:'',notes:cleanText(p.notes,3000),createdAt:Number(p.createdAt)||Date.now(),updatedAt:Number(p.updatedAt)||Date.now()}
}
...
async function connectListingAppointmentToPipeline(details){
  ...
  if(!p){p=normaliseProspect({id:prospectId(),name:details.contactName,phone:details.contactNumber,address:details.address,source:'Listing appointment',stage:'Appointment Booked',temperature:'Warm',sellingTimeframe:'Now',createdAt:Date.now(),updatedAt:Date.now()});prospects.unshift(p);created=true}
...
function setProspectorSection(section='today'){
  prospectSection=['today','contacts','pipeline'].includes(section)?section:'today';
