...
function loadLocal(){try{days=JSON.parse(localStorage.getItem('da-days-v5')||localStorage.getItem('da-days-v4')||'{}');targets={...DEFAULTS,...JSON.parse(localStorage.getItem('da-targets-v5')||localStorage.getItem('da-targets-v4')||'{}')};agentName=localStorage.getItem('da-agent-name')||''}catch{days={};targets={...DEFAULTS};agentName=''}}
function saveLocal(){localStorage.setItem('da-days-v5',JSON.stringify(days));localStorage.setItem('da-targets-v5',JSON.stringify(targets));localStorage.setItem('da-agent-name',agentName)}
...
async function publishLeaderboard(){if(!cloud||!db||!uid)return;try{await setDoc(doc(db,'leaderboard',uid),leaderboardPayload(),{merge:true})}catch(err){console.error('Leaderboard publish failed',err)}}
