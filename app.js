...
function contactDraftMeaningful(draft={}){
  ...
  return Boolean(draft.editingId)||textFields.some(field=>Boolean(values[field]))||values.addBuyerBrief||values.stage!=='New Lead'||values.temperature!=='Cold'||values.motivation!==1
}
...
function cancelContactEditor(){
  const form=$('#prospectEditor');if(form?.dataset.contactDraft==='1'){
    const draft=readContactDraft(),hasChanges=form.dataset.contactDraftEditingId?draft?.editingId===form.dataset.contactDraftEditingId:contactDraftMeaningful(contactDraftFromForm(form));
    if(hasChanges&&!confirm('Discard this unfinished contact?'))return;
    clearContactDraft()
  }
  closeProspectDetail()
}
function restoreContactDraftWorkflow({silent=false}={}){
  ...
  if(draft.editingId&&!prospectById(draft.editingId))return false;switchView('prospectingView');prospectTodayMode='dashboard';setProspectorSection('contacts');openProspectEditor(draft.editingId||'',{draft});if(!silent)toast('Contact draft restored');return true
}
...
function normaliseProspects(list){
  const result=[],seen=new Set();
  for(const raw of Array.isArray(list)?list:[]){
    const prospect=normaliseProspect(raw);
    if(seen.has(prospect.id))continue;
    seen.add(prospect.id);result.push(prospect);
...
function contactEstimateFor(p){
  ...
  const day=todayKey();if(contactEstimateHistoryRef!==marketPulseHistory||contactEstimateEventsRef!==marketPulseEvents||contactEstimateDay!==day){contactEstimateHistoryRef=marketPulseHistory;contactEstimateEventsRef=marketPulseEvents;contactEstimateDay=day;contactEstimateCache.clear();const events=new Map();for(const event of [...normaliseMarketPulseHistory(marketPulseHistory).filter(marketPulseKnockingEventEligible),...normaliseMarketPulseEvents(marketPulseEvents)])if(marketPulseEventKind(event)==='sold')events.set(event.id,event);contactEstimateSource=new Map();for(const event of events.values()){const suburb=normalisePlace(event.suburb),group=contactEstimateSource.get(suburb)||[];group.push(event);contactEstimateSource.set(suburb,group)}}
  const key=JSON.stringify([p.address,p.company,p.suburb,p.sellerPropertyType,p.sellerBedrooms,p.sellerBathrooms,p.sellerCars,p.sellerLandSizeBand,p.sellerTitleType]);
  const cached=contactEstimateCache.get(p.id);if(cached?.key===key)return cached.estimate;
  const addressParts=appointmentMarketAddressParts(p.address||p.company),suburb=normalisePlace(addressParts.suburb||p.suburb),estimate=sellerMarketPulseEstimate(p,contactEstimateSource.get(suburb)||[]);
  contactEstimateCache.set(p.id,{key,estimate});return estimate;
}
...
function renderProspecting(){
  ...
  if(prospectSection==='contacts'&&!detailWasOpen)renderContactsList();
  else if(prospectSection==='buyers')renderBuyerProfiles();
  else if(prospectSection==='pipeline')renderSellerPipeline();
  else if(prospectSection==='insights')renderProspectorInsights();
  else if(prospectSection==='today'&&prospectTodayMode==='followups')renderTodayFollowUpQueue();
...
function openProspectEditor(id='',options={}){const p=id?prospectById(id):{},saved=!id?readContactDraft():null,draft=options.draft||(!saved?.editingId?saved:null);activeProspectId=p?.id||null;$('#prospectingDashboard').classList.add('hidden');$('#prospectingSession').classList.add('hidden');$('#prospectDetail').classList.remove('hidden');$('#prospectDetail').innerHTML=prospectForm(p,draft,options.prefill||{});if(!id&&options.prefill)saveContactDraftFromForm($('#prospectEditor'))}
...
async function saveProspecting({render=true,awaitCloud=true}={}){
  invalidateSellerPriorityCache({delay:80});const localSaved=saveProspectingLocal();if(render)renderProspecting();
  const sync=queueProspectingSave();
  if(!awaitCloud){sync.catch(err=>console.error('Deferred prospecting sync failed',err));return localSaved}
  await sync;return localSaved
}
...
function returnToActiveTabRoot(id){
  if(id==='prospectingView'){
    ...
    if(form?.matches('#prospectEditor[data-contact-draft="1"]')){if(form.dataset.contactDraftEditingId){if(readContactDraft()&&!confirm('Discard unsaved changes and return to My Market?'))return;clearContactDraft()}else if(!saveContactDraftFromForm(form)){toast('Contact draft could not be saved. Keep this screen open.');return}}
...
$('#prospectingView').onsubmit=async e=>{
  ...
  if(e.target.id==='prospectEditor'){
    e.preventDefault();const form=e.target,submit=form.querySelector('button[type=submit]'),editingId=activeProspectId||'',saveId=editingId||cleanText(form.dataset.contactDraftId,80),f=new FormData(form),existing=prospectById(editingId),wasExisting=Boolean(existing),addBuyerBrief=f.get('addBuyerBrief')==='1';if(submit?.disabled)return;window.agntRuntime?.record('contact-save-start');if(submit){submit.disabled=true;submit.textContent='Saving…'}
    try{const result=await upsertProspect({name:f.get('name'),phone:f.get('phone'),email:f.get('email'),address:f.get('address'),source:f.get('source'),stage:f.get('stage'),temperature:f.get('temperature'),motivation:f.get('motivation'),temperatureManual:form.dataset.temperatureManual==='1'||Boolean(existing?.temperatureManual),motivationManual:form.dataset.motivationManual==='1'||Boolean(existing?.motivationManual),tags:f.get('tags'),sellingTimeframe:f.get('sellingTimeframe'),nextFollowUp:f.get('nextFollowUp'),notes:f.get('notes'),archived:Boolean(existing?.archived),archivedAt:existing?.archivedAt||0},saveId);window.agntRuntime?.record('contact-save-complete');clearContactDraft();const message=result.unified?'Buyer and seller records unified':result.reusedExisting&&!wasExisting?'Existing contact reused · no duplicate created':wasExisting?'Contact saved':'Contact added';if(prospectingFormContextIsCurrent(form)){activeProspectId=result.record.id;if(addBuyerBrief){openBuyerEditor(result.record.id);toast('Contact saved. Add their buyer brief.')}else{renderProspectDetail(result.record.id);toast(message)}}else{discardSavedContactEditor(form);toast(message)}}catch(err){window.agntRuntime?.record('contact-save-error',err?.name);console.error('Contact save failed',err);saveContactDraftFromForm(form);toast('Contact could not be saved on this device. Keep this screen open and check sync.');if(submit){submit.disabled=false;submit.textContent=wasExisting?'Save Contact':'Add Contact'}}return
  }
...
$('#exportRuntimeLog').onclick=()=>{const content=window.agntRuntime?.export()||'[]',blob=new Blob([content],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`agnt-diagnostic-${todayKey()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)};
