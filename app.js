...
function renderMarketPulse(){
  ...
  list.innerHTML=events.length?events.map(event=>{
    ...
    if(progress.complete)return`<article class="market-pulse-card market-session-complete-card" data-market-event-id="${escapeHtml(event.id)}">${marketPulseCardHeadingMarkup(event)}<details class="market-pulse-completed-disclosure"><summary><span><strong>Completed</strong><small>${progress.called} call${progress.called===1?'':'s'} · ${progress.sms} SMS</small></span><i aria-hidden="true">›</i></summary><div class="market-pulse-card-content">${details}</div></details></article>`;
