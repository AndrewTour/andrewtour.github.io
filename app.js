...
function normaliseContactMatch(value=''){return String(value||'').trim().toLowerCase().replace(/[^a-z0-9]/g,'')}
function appointmentMatchesProspect(a,p){
  const prospectPhone=normaliseContactMatch(primaryProspectPhone(p)),appointmentPhone=normaliseContactMatch(a.contactNumber||a.phone);
  if(prospectPhone&&appointmentPhone&&prospectPhone===appointmentPhone)return true;
  const prospectName=normaliseContactMatch(p.name),appointmentName=normaliseContactMatch(a.contactName||a.name);
  if(prospectName&&appointmentName&&prospectName===appointmentName)return true;
  const prospectAddress=normaliseContactMatch(p.address),appointmentAddress=normaliseContactMatch(a.address);
  return Boolean(prospectAddress&&appointmentAddress&&prospectAddress===appointmentAddress);
}
function contactTimelineForProspect(p){
  const interactionItems=interactionsFor(p.id).map(x=>({
    key:`interaction-${x.id||x.at}`,
    at:Number(x.at)||new Date(`${x.date||todayKey()}T12:00:00`).getTime(),
    kind:String(x.type||'Contact'),
    title:String(x.outcome||x.type||'Contact logged'),
    note:String(x.note||''),
    nextFollowUp:validDateKey(x.nextFollowUp)?x.nextFollowUp:'',
    date:x.date||dateKey(new Date(Number(x.at)||Date.now()))
  }));
  const appointmentItems=allAppointmentEntries().filter(({appointment})=>appointmentMatchesProspect(appointment,p)).map(({appointment:a,sourceDate,scheduled})=>{
    const lifecycle=appointmentLifecycle(a,sourceDate),type=appointmentType(a),outcome=appointmentOutcomeLabel(a.outcome),scheduledAt=appointmentTimestamp(a,sourceDate);
    const title=outcome?`${type} · ${outcome}`:lifecycle==='completed'?`${type} completed`:lifecycle==='follow-up'?`${type} follow-up`:`${type} booked`;
    const details=[shortAppointmentDate(scheduled),appointmentTimeLabel(a,sourceDate),a.address||''].filter(Boolean).join(' · ');
    const followUp=lifecycle==='follow-up'?(a.followUpDate?followUpDueLabel(a):'Follow-up required'):'';
    return{key:`appointment-${calendarExportId(a,sourceDate)}`,at:scheduledAt||Number(a.createdAt)||Date.now(),kind:'Appointment',title,note:String(a.outcomeNote||''),nextFollowUp:validDateKey(a.followUpDate)?a.followUpDate:'',date:scheduled||sourceDate,details,followUp};
  });
  return[...interactionItems,...appointmentItems].sort((a,b)=>b.at-a.at);
}
function contactTimelineMarkup(items){
  if(!items.length)return'<div class="prospect-empty"><strong>No timeline activity yet</strong><small>Conversations and linked appointments will appear here automatically.</small></div>';
  return items.map(item=>{const time=new Date(item.at);const timeLabel=Number.isNaN(time.getTime())?'':time.toLocaleTimeString('en-AU',{hour:'numeric',minute:'2-digit'});const detail=item.details||`${fmtDate(item.date)}${timeLabel?` · ${timeLabel}`:''}`;const followUp=item.followUp||(item.nextFollowUp?`Follow-up: ${fmtDate(item.nextFollowUp)}`:'');return`<article class="contact-timeline-item kind-${escapeHtml(item.kind.toLowerCase().replace(/[^a-z0-9]+/g,'-'))}"><i></i><div><span class="contact-timeline-kind">${escapeHtml(item.kind)}</span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(detail)}</small>${item.note?`<p>${escapeHtml(item.note)}</p>`:''}${followUp?`<em>${escapeHtml(followUp)}</em>`:''}</div></article>`}).join('');
}
function renderProspectDetail(id){const p=prospectById(id);if(!p)return closeProspectDetail();activeProspectId=id;const history=interactionsFor(id),timeline=contactTimelineForProspect(p),phone=primaryProspectPhone(p),tel=prospectTel(p),sms=phone?`sms:${phone.replace(/[^+\d]/g,'')}`:'#';$('#prospectDetail').innerHTML=`<div class="prospect-detail-nav"><button type="button" data-close-prospect>‹ Back</button><button type="button" data-edit-prospect="${p.id}">Edit</button></div><section class="prospect-profile glass"><div class="prospect-profile-top"><span class="prospect-avatar large">${escapeHtml(p.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase())}</span><div><span>${escapeHtml(p.stage)}</span><h2>${escapeHtml(p.name)}</h2><small>${escapeHtml(formatProspectAddress(p.address||p.company,p.suburb)||'No address added')}</small></div><span class="prospect-temp temp-${p.temperature.toLowerCase()}">${p.temperature}</span></div><div class="prospect-quick-actions"><a href="${tel}" class="${phone?'':'disabled'}">Call</a><a href="${sms}" class="${phone?'':'disabled'}">Message</a><button type="button" data-log-prospect="${p.id}">Log Contact</button></div><div class="prospect-profile-grid"><div><span>NEXT FOLLOW-UP</span><strong>${p.nextFollowUp?fmtDate(p.nextFollowUp):'Not set'}</strong></div><div><span>LAST CONTACT</span><strong>${p.lastContact?fmtDate(p.lastContact):'Never'}</strong></div><div><span>MOTIVATION</span><strong>${p.motivation}/5</strong></div><div><span>CONTACTS</span><strong>${history.length}</strong></div></div>${pipelineTimeframeForProspect(p)?`<div class="prospect-selling-status"><span>SELLING TIMEFRAME</span><strong>${escapeHtml(pipelineTimeframeForProspect(p))}</strong>${pipelineAppointmentLabel(p)?`<small>${escapeHtml(pipelineAppointmentLabel(p))}</small>`:''}</div>`:''}${p.tags.length?`<div class="prospect-tags">${p.tags.map(t=>`<span>${escapeHtml(t)}</span>`).join('')}</div>`:''}${p.notes?`<p class="prospect-background">${escapeHtml(p.notes)}</p>`:''}</section><section class="prospecting-section glass"><div class="prospecting-section-head"><div><span>CONTACT TIMELINE</span><h3>Complete activity</h3></div></div><div class="prospect-history contact-timeline">${contactTimelineMarkup(timeline)}</div></section><button class="prospect-delete" type="button" data-delete-prospect="${p.id}">Delete Contact</button>`}
