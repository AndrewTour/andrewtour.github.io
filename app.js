...
function renderToday(){
  ...
  if(offDayOpportunityMode)renderOffDayConversations();
  if($('#welcomeMessage')){
...
function offDayConversationState(date=todayKey()){
  const contacted=new Set(),doNotContact=new Set(),lastAt=new Map();
  for(const item of prospectInteractions){const id=cleanText(item?.prospectId,80);if(!id)continue;const at=Number(item.at)||0;if(at>Number(lastAt.get(id)||0))lastAt.set(id,at);if(item.outcome==='Do not contact')doNotContact.add(id);if(item.date===date&&['Call','SMS','Appointment','Follow-up','Buyer match'].includes(item.type))contacted.add(id)}
  return{contacted,doNotContact,lastAt}
}
...
function offDayConversationCandidates(limit=3){
  ...
  for(const prospect of candidatePool){
    if(state.contacted.has(prospect.id)||state.doNotContact.has(prospect.id))continue;const followDistance=prospect.nextFollowUp?sellerPriorityDateDistance(prospect.nextFollowUp,today):null,overdueDays=followDistance===null?0:Math.max(0,followDistance),dueToday=prospect.nextFollowUp===today,timeframe=sellerPriorityTimeframe(prospect),sellerSignal=Boolean(timeframe||prospect.sellerProfileActive||['Appraisal Opportunity','Appointment Booked','Pipeline'].includes(prospect.stage)),buyerSignal=prospectHasActiveBuyerRole(prospect),lastAt=Number(state.lastAt.get(prospect.id)||0),daysSince=lastAt?Math.max(0,Math.floor((now.getTime()-lastAt)/86400000)):null,pipelineReady=!prospect.nextFollowUp&&(daysSince===null||daysSince>=21);let score=0,reason='';
    ...
    if(score<60)continue;const role=offDayConversationRole(prospect,sellerSignal);add({id:`contact-${prospect.id}`,prospectId:prospect.id,name:prospect.name,role,reason,score,action:buyerSignal&&!sellerSignal?'open-buyer':'open-seller',eventId:prospect.id,phone:primaryProspectPhone(prospect)})
  }
...
function renderOffDayConversations(){
  ...
  host.innerHTML=items.map(item=>{const initials=item.name.split(/\s+/).slice(0,2).map(part=>part[0]||'').join('').toUpperCase()||'A';return`<article class="off-day-conversation-row"><button class="off-day-conversation-profile" type="button" data-plan-action="${escapeHtml(item.action)}" data-event-id="${escapeHtml(item.eventId||'')}"><span class="prospect-avatar">${escapeHtml(initials)}</span><span class="off-day-conversation-copy"><span>${escapeHtml(item.role)}</span><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.reason)}</small></span><b aria-hidden="true">›</b></button>${offDayConversationCallMarkup(item)}</article>`}).join('')
}
