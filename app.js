...
function contactValueMarkup(p){const estimate=contactEstimateFor(p);if(!estimate?.available)return sellerPropertyConfigured(p)?'<span class="contact-value">MarketPulse estimate building</span>':'';const trend=estimate.previousMedian?estimate.median>estimate.previousMedian?'up':estimate.median<estimate.previousMedian?'down':'neutral':'neutral';const glyph=trend==='neutral'?'−':`<svg viewBox="0 0 16 16" width="14" height="14" aria-hidden="true"><path d="M8 ${trend==='up'?'13V3M3 8l5-5 5 5':'3v10m-5-5 5 5 5-5'}" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>`;return`<span class="contact-value" title="${trend==='neutral'?'No comparable movement confirmed':'Change after the latest comparable sale'}"><span class="contact-value-trend trend-${trend}" aria-label="${trend==='up'?'Estimate up':trend==='down'?'Estimate down':'No confirmed estimate movement'}">${glyph}</span><span>MarketPulse ${escapeHtml(formatMarketMoney(estimate.low))}–${escapeHtml(formatMarketMoney(estimate.high))}</span></span>`}
...
function jumpToContactLetter(letter){if(!/^[A-Z]$/.test(letter))return;const row=$('#prospectContactList')?.querySelector(`[data-contact-letter="${letter}"]`),scroller=$('#prospectingView');if(!row||!scroller)return;const offset=row.getBoundingClientRect().top-scroller.getBoundingClientRect().top+scroller.scrollTop-(scroller.querySelector('.prospecting-toolbar')?.getBoundingClientRect().height||0)-8;scroller.scrollTo({top:Math.max(0,offset),behavior:'instant'})}
const contactAlphabet=$('#contactAlphabet');if(contactAlphabet){contactAlphabet.addEventListener('click',event=>jumpToContactLetter(event.target.closest('[data-contact-jump]')?.dataset.contactJump||''));contactAlphabet.addEventListener('pointerdown',event=>{if(event.pointerType==='mouse')return;event.preventDefault();jumpToContactLetter(document.elementFromPoint(event.clientX,event.clientY)?.closest('[data-contact-jump]')?.dataset.contactJump||'')});contactAlphabet.addEventListener('pointermove',event=>{if(event.pointerType==='mouse'||!event.buttons)return;event.preventDefault();jumpToContactLetter(document.elementFromPoint(event.clientX,event.clientY)?.closest('[data-contact-jump]')?.dataset.contactJump||'')})}
