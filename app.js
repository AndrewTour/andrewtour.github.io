...
function marketSessionProgress(event,matches=marketMatches(event)){
  ...
  return{total:matches.length,called:Math.min(workedIds.size,matches.length),connects:connectedIds.size,followUps:followUpIds.size,active,complete,workedIds};
}
