...
function marketPulseCardHeadingMarkup(event){return`<div class="market-pulse-card-head"><div class="market-pulse-card-title"><h4>${escapeHtml(event.address)}</h4><p>${escapeHtml(event.suburb)}</p></div><div class="market-pulse-card-tools"><span class="market-event-tag event-${marketTypeClass(event.eventType)}">${escapeHtml(event.eventType)}</span><button type="button" data-remove-market-event="${escapeHtml(event.id)}" aria-label="Remove ${escapeHtml(event.address)}">×</button></div></div>`}
function renderMarketPulse(){
  ...
  list.innerHTML=events.length?events.map(event=>{
    ...
    if(progress.complete)return`<article class="market-pulse-card market-session-complete-card" data-market-event-id="${escapeHtml(event.id)}">${marketPulseCardHeadingMarkup(event)}<details class="market-pulse-completed-disclosure"><summary><span><strong>Completed activity</strong><small>${progress.called} call${progress.called===1?'':'s'} · ${progress.sms} SMS</small></span><span class="market-pulse-complete-state"><strong>Complete</strong><i aria-hidden="true">›</i></span></summary><div class="market-pulse-card-content">${details}</div></details></article>`;
