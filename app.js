...
function startProspectingSession(){const due=priorityProspects().filter(p=>p.nextFollowUp&&p.nextFollowUp<=todayKey());prospectSessionIds=(due.length?due:priorityProspects()).slice(0,50).map(p=>p.id);prospectSessionIndex=0;prospectSessionActive=true;prospectSessionStats={calls:0,connects:0,temperate:0,appointments:0};if(!prospectSessionIds.length){prospectSessionActive=false;return toast('Add a prospect to begin')}showProspectingSession()}
...
$('#prospectingView').onclick=async e=>{
  ...
  if(e.target.closest('[data-session-skip]')){prospectSessionIndex++;showProspectingSession();return}
};
