...
function timelineItemsForDate(viewDate){
  ...
  const items=[...workAllocation.items,...appointmentModel.items,...knockAllocation.items,...(progressItem?[progressItem]:[]),wrapItem],order={appointment:0,ofi:0,market:1,'followup-block':2,'pipeline-block':3,knock:4,'knock-waiting':5,check:6,wrap:7};
  return items.sort((a,b)=>a.minutes-b.minutes||(order[a.kind]??9)-(order[b.kind]??9)||a.title.localeCompare(b.title));
}
...
function dailyCommandPriority(viewDate=selectedDate,items=timelineItemsForDate(viewDate),now=new Date()){
  ...
  const attention=actionable.filter(item=>nowMinutes>=item.minutes+(item.duration||15)).sort((a,b)=>{const priority={market:0,'followup-block':1,'pipeline-block':2,knock:3,appointment:4,ofi:4};return(priority[a.kind]??9)-(priority[b.kind]??9)||a.minutes-b.minutes})[0];
  ...
  unsubProspecting=onSnapshot(doc(db,'users',uid,'prospecting','state'),{includeMetadataChanges:true},snap=>{
    if(snap.exists()){
      ...
      if(nextSignature!==prospectingSignature()){
        prospects=nextProspects;prospectInteractions=nextInteractions;marketPulseEvents=nextMarketEvents;marketPulseHistory=nextMarketHistory;saveLocal();renderProspecting();renderMarketPulse();refreshReturningSnapshotIfVisible()
      }
