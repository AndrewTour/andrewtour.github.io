...
async function updateAppointmentRecord(id,sourceDate,changes){
  ...
  days[sourceDate]=d;await saveDay(sourceDate);const updated=d.appointments[index],eventType=changes.outcome?'appointment.outcome':changes.followUpDate?'appointment.followup_scheduled':changes.followedUpAt?'appointment.followup_completed':'appointment.updated',outcome=changes.outcome|| (changes.followUpDate?'Follow-up scheduled':changes.followedUpAt?'Follow-up completed':'Appointment updated');recordAppointmentIntegration(updated,sourceDate,eventType,outcome,changes.outcomeNote||'');if(linkedProspectForAppointment(updated))await saveProspecting({render:false});renderAll();
}
...
async function addAppointment({contactName,contactNumber,address,date,time,type}){
  ...
  days[createdDate]=d;
  try{await saveDay(createdDate);recordAppointmentIntegration(appointment,createdDate,'appointment.booked','Appointment booked',`${appointmentType(appointment)} booked for ${shortAppointmentDate(date)} at ${appointmentTimeLabel(appointment,createdDate)}.`);if(linkedProspectForAppointment(appointment))await saveProspecting({render:false});renderAppointments();toast(date===createdDate?'Appointment logged':'Appointment logged and reminder created');return appointment}
...
async function editAppointment({contactName,contactNumber,address,date,time,type}){
  ...
  addEvent(d,'appointment',`${type} · ${contactName} · appointment updated for ${date} ${time}`);days[sourceDate]=d;await saveDay(sourceDate);recordAppointmentIntegration(d.appointments[index],sourceDate,'appointment.updated','Appointment updated',`${type} updated for ${shortAppointmentDate(date)} at ${appointmentTimeLabel(d.appointments[index],sourceDate)}.`);if(linkedProspectForAppointment(d.appointments[index]))await saveProspecting({render:false});editingAppointment=null;renderAll();toast('Appointment updated');return d.appointments[index];
}
async function deleteAppointment(id,sourceDate=appointmentDate){
  ...
  recordAppointmentIntegration(appointment,sourceDate,'appointment.deleted','Appointment deleted','Appointment and associated follow-up removed.');d.appointments.splice(index,1);days[sourceDate]=d;
  const ids=calendarExportIds();ids.delete(exportId);localStorage.setItem(calendarExportStorageKey(),JSON.stringify([...ids]));
  await saveDay(sourceDate);if(linkedProspectForAppointment(appointment))await saveProspecting({render:false});renderAll();toast('Appointment deleted');
}
...
function normaliseProspectInteractions(list){return(Array.isArray(list)?list:[]).filter(x=>x&&typeof x==='object').map(x=>({id:cleanText(x.id,80)||prospectId(),prospectId:cleanText(x.prospectId,80),date:validDateKey(x.date)?x.date:todayKey(),at:Number(x.at)||Date.now(),type:cleanText(x.type,40)||'Note',outcome:cleanText(x.outcome,80),note:cleanText(x.note,2000),nextFollowUp:validDateKey(x.nextFollowUp)?x.nextFollowUp:'',eventType:cleanText(x.eventType,60),entityType:cleanText(x.entityType,40),entityId:cleanText(x.entityId,100),source:cleanText(x.source,60),previousValue:cleanText(x.previousValue,160),newValue:cleanText(x.newValue,160),scheduledDate:validDateKey(x.scheduledDate)?x.scheduledDate:'',appointmentType:cleanText(x.appointmentType,20),metricsApplied:Boolean(x.metricsApplied)})).filter(x=>x.prospectId).slice(-20000)}
...
function integrationEventExists(prospectId,eventType,entityId=''){return prospectInteractions.some(x=>x.prospectId===prospectId&&x.eventType===eventType&&(!entityId||x.entityId===String(entityId)))}
function recordIntegrationEvent(contactId,{eventType='',entityType='',entityId='',type='Note',outcome='',note='',nextFollowUp='',at=Date.now(),date=todayKey(),source='AGNT',previousValue='',newValue='',scheduledDate='',appointmentType:''}={}){
  if(!contactId||!prospectById(contactId))return null;
  const id=entityId?`evt_${eventType}_${String(entityId)}`.replace(/[^a-zA-Z0-9_-]/g,'_').slice(0,100):prospectId();
  if(entityId&&integrationEventExists(contactId,eventType,String(entityId)))return prospectInteractions.find(x=>x.prospectId===contactId&&x.eventType===eventType&&x.entityId===String(entityId));
  const event=normaliseProspectInteractions([{id,prospectId:contactId,date,at,type,outcome,note,nextFollowUp,eventType,entityType,entityId:String(entityId||''),source,previousValue,newValue,scheduledDate,appointmentType}])[0];
  if(event)prospectInteractions.push(event);return event||null;
}
function linkedProspectForAppointment(appointment){return appointment?.prospectId?prospectById(appointment.prospectId):findProspectForAppointment({contactName:appointment?.contactName||appointment?.name||'',contactNumber:appointment?.contactNumber||appointment?.phone||'',address:appointment?.address||''})}
function recordAppointmentIntegration(appointment,sourceDate,eventType,outcome,note='',extra={}){
  const p=linkedProspectForAppointment(appointment);if(!p)return null;
  const scheduledDate=appointmentScheduledDate(appointment,sourceDate),kind=appointmentType(appointment);
  const eventEntityId=eventType==='appointment.booked'?appointment.id:`${appointment.id}_${Number(appointment.updatedAt||Date.now())}`;
  const event=recordIntegrationEvent(p.id,{eventType,entityType:'appointment',entityId:eventEntityId,type:'Appointment',outcome,note,nextFollowUp:appointment.followUpDate||'',at:Number(appointment.updatedAt||appointment.at)||Date.now(),date:validDateKey(sourceDate)?sourceDate:todayKey(),source:'Appointments',scheduledDate,appointmentType:kind,...extra});
  prospects=prospects.map(x=>x.id===p.id?normaliseProspect({...x,stage:kind==='LAP'&&['appointment.booked','appointment.updated'].includes(eventType)?'Appointment Booked':x.stage,updatedAt:Date.now()}):x);
  return event;
}
function backfillIntegrationEvents(){
  let added=0;
  for(const {appointment,sourceDate} of allAppointmentEntries()){
    const p=linkedProspectForAppointment(appointment);if(!p||integrationEventExists(p.id,'appointment.booked',String(appointment.id)))continue;
    const scheduledDate=appointmentScheduledDate(appointment,sourceDate),kind=appointmentType(appointment),at=Number(appointment.at||appointment.createdAt)||new Date(`${sourceDate}T09:00:00`).getTime();
    const event=recordIntegrationEvent(p.id,{eventType:'appointment.booked',entityType:'appointment',entityId:appointment.id,type:'Appointment',outcome:'Appointment booked',note:`${kind} booked for ${shortAppointmentDate(scheduledDate)} at ${appointmentTimeLabel(appointment,sourceDate)}.`,at,date:appointmentCreatedDate(appointment,sourceDate)||sourceDate,source:'Appointments',scheduledDate,appointmentType:kind});
    if(event)added++;
  }
  return added;
}
...
function completeProspectFollowUp(id){const p=prospectById(id);if(!p)return;openProspectLog(id,false,true)}
...
function upsertProspect(data,id=''){const existing=id?prospectById(id):null;const record=normaliseProspect({...existing,...data,id:id||prospectId(),createdAt:existing?.createdAt||Date.now(),updatedAt:Date.now()});if(existing)prospects=prospects.map(p=>p.id===id?record:p);else prospects.unshift(record);if(existing&&existing.sellingTimeframe!==record.sellingTimeframe)recordIntegrationEvent(record.id,{eventType:'pipeline.timeframe_changed',entityType:'contact',entityId:`${record.id}_${Date.now()}`,type:'Pipeline',outcome:'Selling timeframe updated',note:`Selling timeframe changed from ${existing.sellingTimeframe||'Not set'} to ${record.sellingTimeframe||'Not currently selling'}.`,previousValue:existing.sellingTimeframe||'',newValue:record.sellingTimeframe||'',source:'Contacts'});recordIntegrationEvent(record.id,{eventType:existing?'contact.updated':'contact.created',entityType:'contact',entityId:`${record.id}_${record.updatedAt}`,type:'Contact',outcome:existing?'Contact updated':'Contact created',note:existing?'Contact details updated.':'Contact added to AGNT.',source:'Contacts'});activeProspectId=record.id;return saveProspecting().then(()=>renderProspectDetail(record.id))}
function openProspectLog(id,fromSession=false,completingFollowUp=false){const p=prospectById(id);if(!p)return;$('#prospectDetail').classList.remove('hidden');$('#prospectingDashboard').classList.add('hidden');$('#prospectingSession').classList.add('hidden');activeProspectId=id;$('#prospectDetail').innerHTML=`<form id="prospectLogForm" class="prospect-editor glass" data-from-session="${fromSession?'1':'0'}" data-completing-followup="${completingFollowUp?'1':'0'}"><div class="prospect-detail-nav"><button type="button" data-cancel-log>‹ Back</button><strong>${completingFollowUp?'Complete Follow-Up':'Log Contact'}</strong><span></span></div><div class="prospect-log-person"><span>${escapeHtml(p.name)}</span><small>${escapeHtml(primaryProspectPhone(p)||p.address||'')}</small></div><label>Outcome<select name="outcome"><option>Connected</option><option>No answer</option><option>Left voicemail</option><option>Sent SMS</option><option>Appraisal opportunity</option><option>Appointment booked</option><option>Not interested</option><option>Do not contact</option></select></label><label>Conversation note<textarea name="note" rows="5" placeholder="What changed? What matters next?"></textarea></label><div class="prospect-form-grid"><label>Temperature<select name="temperature" data-pipeline-temperature-field>${['Cold','Warm','Hot'].map(x=>`<option ${p.temperature===x?'selected':''}>${x}</option>`).join('')}</select></label><label>Next follow-up<input name="nextFollowUp" type="date" value="${completingFollowUp?'':p.nextFollowUp||''}"></label></div><label>Selling timeframe<select name="sellingTimeframe" data-pipeline-timeframe-field><option value="">Leave unchanged</option>${SELLING_TIMEFRAMES.map(x=>`<option value="${x}">${x}</option>`).join('')}<option value="Not currently selling">Not currently selling</option></select></label><button class="primary" type="submit">Save & ${fromSession?'Next':'Finish'}</button></form>`}
...
$('#prospectingView').onclick=async e=>{
  ...
  const completeFollowUp=e.target.closest('[data-complete-prospect-followup]');if(completeFollowUp){completeProspectFollowUp(completeFollowUp.dataset.completeProspectFollowup);return}
