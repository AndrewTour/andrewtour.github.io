...
function sellerPriorityMarketSignal(prospect={}){
  const byId=new Map();for(const event of normaliseMarketPulseEvents([...marketPulseHistory,...marketPulseEvents]))byId.set(event.id,event);return[...byId.values()].map(event=>({event,days:sellerPriorityDateDistance(event.receivedDate)})).filter(row=>row.days>=0&&row.days<=14&&marketMatches(row.event).some(person=>person.id===prospect.id)).sort((a,b)=>hotSpottingPriority(b.event,1).score-hotSpottingPriority(a.event,1).score||a.days-b.days)[0]||null
}
function sellerPriorityCandidate(prospect={},now=new Date(),deferrals=readSellerPriorityDeferrals(now.getTime())){
  if(!prospect||prospect.archived||prospect.stage==='Listed'||!primaryProspectPhone(prospect)||prospectContactedToday(prospect.id)||interactionsFor(prospect.id).some(item=>item.date===todayKey()&&['SMS','Appointment','Follow-up'].includes(item.type)||item.outcome==='Do not contact'))return null;
  const activeDeferral=deferrals.find(row=>row.prospectId===prospect.id&&Number(row.until)>now.getTime());if(activeDeferral)return null;
  const timeframe=pipelineTimeframeForProspect(prospect),followUpDistance=prospect.nextFollowUp?sellerPriorityDateDistance(prospect.nextFollowUp):null,overdueDays=followUpDistance===null?0:Math.max(0,followUpDistance),dueToday=followUpDistance===0&&prospect.nextFollowUp===todayKey(),market=sellerPriorityMarketSignal(prospect),stage=cleanText(prospect.stage,60),lastContactDays=prospect.lastContact?Math.max(0,sellerPriorityDateDistance(prospect.lastContact)):null;
  ...
  const timeframeWeight={'Now':32,'1–3 months':24,'6–12 months':10,'12 months+':4}[timeframe]||0,temperatureWeight={Hot:24,Warm:12,Cold:2}[prospect.temperature]||0,stageWeight={'Appraisal Opportunity':28,'Appointment Booked':22,'Pipeline':12,'Past Client':7}[stage]||0,followUpWeight=overdueDays?34+Math.min(18,overdueDays*2):dueToday?30:prospect.nextFollowUp&&sellerPriorityDateDistance(prospect.nextFollowUp)<0&&sellerPriorityDateDistance(prospect.nextFollowUp)>=-3?8:0,marketWeight=market?Math.min(22,12+Math.round(hotSpottingPriority(market.event,1).score/8)):0,freshnessWeight=lastContactDays===null?10:lastContactDays>=14?12:lastContactDays>=7?7:lastContactDays<=1&&!dueToday?-12:0,repeatDeferrals=deferrals.filter(row=>row.prospectId===prospect.id).length,score=Number(prospect.motivation||1)*4+timeframeWeight+temperatureWeight+stageWeight+followUpWeight+marketWeight+freshnessWeight+4-Math.min(18,repeatDeferrals*6);
...
function sellerNextBestAction(now=new Date()){
  if(selectedDate!==todayKey())return null;const deferrals=readSellerPriorityDeferrals(now.getTime()),winner=activeProspects().map(prospect=>sellerPriorityCandidate(prospect,now,deferrals)).filter(Boolean).filter(item=>item.score>=44).sort((a,b)=>b.score-a.score||(a.prospect.nextFollowUp||'9999-12-31').localeCompare(b.prospect.nextFollowUp||'9999-12-31')||b.prospect.updatedAt-a.prospect.updatedAt)[0];if(!winner)return null;
  return{title:`Call ${winner.prospect.name} — ${winner.purpose}`,meta:winner.reasons.join(' · '),kicker:'SELLER PRIORITY',timeLabel:'10 min',focusItemId:'',action:'open-seller',label:'Open seller',eventId:winner.prospect.id,sellerPriority:true}
}
...
function deferSellerPriority(hours='',reason=''){
  if(!pendingSellerPriorityId)return;const now=Date.now(),duration=Number(hours),endOfToday=new Date(now);endOfToday.setHours(23,59,59,999);const until=hours==='workday'?nextSellerPriorityWorkdayStart(new Date(now)):hours==='today'?endOfToday.getTime():now+Math.max(1,duration)*60*60*1000,rows=readSellerPriorityDeferrals(now);rows.push({prospectId:pendingSellerPriorityId,reason:cleanText(reason,80),at:now,until});writeSellerPriorityDeferrals(rows);closeSellerPriorityDeferral();renderNowCard();renderTimeline();toast(hours==='workday'?'Moved to the next workday':hours==='today'?'Moved out of today':'Remind you again in 2 hours')
}
...
function sellerMarketPulseEstimate(p={}){
  ...
  const comps=[...byId.values()].filter(event=>marketPulseEventKind(event)==='sold'&&normalisePlace(event.suburb)===suburb&&parseMarketMoney(event.price)>0).map(event=>({...event,similarity:sellerMarketSimilarity(p,event),priceValue:parseMarketMoney(event.price),age:relativeEventRecency(event).days})).filter(event=>event.age<=365&&event.similarity.evidence>0&&event.similarity.ratio>=.45).sort((a,b)=>b.similarity.ratio-a.similarity.ratio||a.age-b.age).slice(0,8);
  if(!comps.length)return{available:false,reason:'No comparable MarketPulse sold results are available yet.'};
  const values=comps.map(comp=>comp.priceValue).sort((a,b)=>a-b),median=marketMedianValue(values),minimumWidth=.05,rawLow=values.length===1?median*.925:values[Math.floor((values.length-1)*.25)],rawHigh=values.length===1?median*1.075:values[Math.ceil((values.length-1)*.75)],low=Math.floor(Math.min(rawLow,median*(1-minimumWidth))/10000)*10000,high=Math.ceil(Math.max(rawHigh,median*(1+minimumWidth))/10000)*10000,averageSimilarity=comps.reduce((total,comp)=>total+comp.similarity.ratio,0)/comps.length,confidence=comps.length>=5&&averageSimilarity>=.8?'High':comps.length>=3&&averageSimilarity>=.65?'Medium':'Low';
  return{available:true,low,high,median,confidence,count:comps.length,averageSimilarity,suburb:comps[0]?.suburb||addressParts.suburb};
}
function sellerEstimateMarkup(p={}){if(!sellerPropertyConfigured(p))return'';const estimate=sellerMarketPulseEstimate(p);if(!estimate?.available)return`<section class="seller-estimate seller-estimate-empty"><span>MARKETPULSE ESTIMATE</span><strong>Estimate building</strong><small>${escapeHtml(estimate?.reason||'More sold evidence is required.')}</small></section>`;return`<section class="seller-estimate"><div><span>MARKETPULSE ESTIMATE</span><strong>${escapeHtml(formatMarketMoney(estimate.low))}–${escapeHtml(formatMarketMoney(estimate.high))}</strong></div><em class="confidence-${estimate.confidence.toLowerCase()}">${escapeHtml(estimate.confidence)} confidence</em><small>${estimate.count} comparable sold result${estimate.count===1?'':'s'} · ${escapeHtml(estimate.suburb)} · last 12 months</small><p>Approximate only, based solely on the closest MarketPulse sold configurations.</p></section>`}
...
function sellerPropertyProfileMarkup(p={}){if(!sellerPropertyConfigured(p))return'';return`<section class="seller-profile-config"><span>PROPERTY DETAILS</span><strong>${escapeHtml(sellerConfigurationLabel(p))}</strong></section>${sellerEstimateMarkup(p)}`}
function sellerPropertyActionMarkup(p={}){const configured=sellerPropertyConfigured(p),summary=configured?sellerConfigurationLabel(p):'Beds, baths, cars, property type, land size and title';return`<button class="device-contact-action property-details-action" type="button" data-open-seller-property="${escapeHtml(p.id)}"><span><strong>${configured?'Edit Property Details':'Add Property Details'}</strong><small>${escapeHtml(summary)}</small></span><b aria-hidden="true">›</b></button>`}
...
async function startCloud(user,{promptTeamSetup=false}={}){
  ...
  unsubProspecting=onSnapshot(doc(db,'users',uid,'prospecting','state'),{includeMetadataChanges:true},snap=>{
    if(snap.exists()){
      const data=snap.data(),cloudProspects=normaliseProspects(data.prospects),localSellerMerge=preserveNewerLocalSellerDetails(cloudProspects),nextProspects=localSellerMerge.prospects,nextInteractions=normaliseProspectInteractions(data.interactions),hasMarketEvents=Object.prototype.hasOwnProperty.call(data,'marketPulseEvents'),hasMarketHistory=Object.prototype.hasOwnProperty.call(data,'marketPulseHistory'),cloudMarketEvents=hasMarketEvents?normaliseMarketPulseEvents(data.marketPulseEvents):[],cloudMarketHistory=hasMarketHistory?normaliseMarketPulseHistory(data.marketPulseHistory):[],cloudSignature=prospectingSignature(cloudProspects,nextInteractions,cloudMarketEvents,cloudMarketHistory);
...
function handleAppSuspend(){captureWorkspaceState();persistOpenContactDraft();if(buyerSession.active){const host=$('#prospectingSession');buyerSession.visible=Boolean(buyerSession.visible&&activeViewId()==='prospectingView'&&host?.dataset.sessionKind==='buyer');saveBuyerSession()}if(pendingProspectingPayload)flushProspectingSave()}
...
$('#sellerPriorityDeferralModal')?.addEventListener('click',e=>{if(e.target.id==='sellerPriorityDeferralModal')closeSellerPriorityDeferral();const choice=e.target.closest('[data-seller-deferral-hours]');if(choice)deferSellerPriority(choice.dataset.sellerDeferralHours,choice.dataset.sellerDeferralReason)});
...
$('#prospectingView').onclick=async e=>{
  ...
  const open=e.target.closest('[data-open-prospect]');if(open){sellerPriorityReturnView='';$('#prospectingDashboard').classList.add('hidden');$('#prospectDetail').classList.remove('hidden');renderProspectDetail(open.dataset.openProspect);return}
...
$('#prospectingView').onsubmit=async e=>{
  if(e.target.id==='sellerPropertyEditor'){
    ...
    try{await saveProspecting({render:false,awaitCloud:true});renderProspectDetail(updated.id);toast(sellerPropertyConfigured(updated)?'Property details saved':'Property details cleared')}catch(err){console.error('Property details save failed',err);toast('Property details saved locally. Please check sync.');renderProspectDetail(updated.id)}return
  }
