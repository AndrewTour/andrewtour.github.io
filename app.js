...
function deferSellerPriority(hours='',reason=''){
  if(!pendingSellerPriorityId)return;const now=Date.now(),duration=Number(hours),endOfToday=new Date(now);endOfToday.setHours(23,59,59,999);const until=hours==='workday'?nextSellerPriorityWorkdayStart(new Date(now)):hours==='today'?endOfToday.getTime():now+Math.max(1,duration)*60*60*1000,rows=readSellerPriorityDeferrals(now);rows.push({prospectId:pendingSellerPriorityId,reason:cleanText(reason,80),at:now,until});writeSellerPriorityDeferrals(rows);closeSellerPriorityDeferral();invalidateSellerPriorityCache({delay:0});renderNowCard();renderTimeline();toast(hours==='workday'?'Moved to the next workday':hours==='today'?'Moved out of today':'Remind you again in 2 hours')
}
...
function cancelProspectAppointmentFlow(){
  ...
  if(flow?.prospectId&&!flow.buyerMatchId)clearProspectCallReturn(flow.prospectId);
...
const PROSPECT_CALL_RETURN_LEGACY_KEY='agnt-prospect-call-return',PROSPECT_CALL_RETURN_MAX_AGE=2*60*60*1000;
function prospectCallReturnStorageKey(userId=uid){return`${storagePrefix(userId||currentUser?.uid||'device')}prospect-call-return:v2`}
function readProspectCallReturn(){let pending=null;try{pending=safeJsonParse(localStorage.getItem(prospectCallReturnStorageKey())||'null',null)}catch(err){console.warn('Call return state could not be read',err)}if(pending?.id)return pending;const canMigrate=Boolean(currentUser?.uid||(uid&&uid!=='local'));if(!canMigrate)return null;try{pending=safeJsonParse(sessionStorage.getItem(PROSPECT_CALL_RETURN_LEGACY_KEY)||'null',null);if(pending?.id){localStorage.setItem(prospectCallReturnStorageKey(),JSON.stringify(pending));sessionStorage.removeItem(PROSPECT_CALL_RETURN_LEGACY_KEY)}}catch(err){console.warn('Legacy call return state could not be restored',err)}return pending?.id?pending:null}
function clearProspectCallReturn(id=''){const pending=readProspectCallReturn();if(id&&pending?.id&&pending.id!==id)return false;try{localStorage.removeItem(prospectCallReturnStorageKey());sessionStorage.removeItem(PROSPECT_CALL_RETURN_LEGACY_KEY)}catch{}return true}
function rememberProspectCallReturn(id,fromSession=false,returnMode=''){if(!id)return;const pending={id,fromSession:Boolean(fromSession),returnMode:cleanText(returnMode,40),at:Date.now()};try{localStorage.setItem(prospectCallReturnStorageKey(),JSON.stringify(pending));sessionStorage.removeItem(PROSPECT_CALL_RETURN_LEGACY_KEY)}catch(err){console.warn('Call return state could not be saved',err)}}
function resumeProspectCallReturn(){const pending=readProspectCallReturn();if(!pending?.id)return false;const age=Date.now()-(Number(pending.at)||0);if(age<350)return false;if(age>PROSPECT_CALL_RETURN_MAX_AGE){clearProspectCallReturn(pending.id);return false}const p=prospectById(pending.id);if(!p)return false;const form=$('#prospectLogForm'),detail=$('#prospectDetail');if(form&&activeProspectId===pending.id&&detail&&!detail.classList.contains('hidden'))return true;switchView('prospectingView');openProspectLog(pending.id,Boolean(pending.fromSession),{returnMode:pending.returnMode});return true}
...
function showApp(){
  ...
  requestAnimationFrame(()=>{try{restoreProspectingSessionState();restoreKnockingSessionState();renderKnockingSession();renderAll();ensureTick();const resumedExternal=resumePendingExternalAction(),restoredDraft=resumedExternal?false:restoreContactDraftWorkflow({silent:true});if(!resumedExternal&&!restoredDraft&&resumeBuyer){setProspectorSection('today');switchView('prospectingView');showBuyerSession()}else if(!resumedExternal&&!restoredDraft)restoreWorkspaceState();$('#appointmentDatePicker').value=appointmentDate;if(!resumedExternal){showLaunchExperience();requestAnimationFrame(()=>maybeShowTeamAppointmentNotice())}}catch(err){console.error('Post-startup rendering failed',err);ensureTick();toast('AGNT opened. Some live panels are still refreshing.')}})
}
let viewportFrame=0;
...
$('#prospectingView').onclick=async e=>{
  ...
  if(e.target.closest('[data-cancel-log]')){const form=$('#prospectLogForm');clearProspectCallReturn(activeProspectId);if(prospectSessionActive&&form?.dataset.fromSession==='1')showProspectingSession();else if(form?.dataset.returnMode==='followups'){activeProspectId=null;prospectTodayMode='followups';$('#prospectDetail').classList.add('hidden');$('#prospectDetail').innerHTML='';$('#prospectingDashboard').classList.remove('hidden');renderProspecting()}else renderProspectDetail(activeProspectId);return}
...
$('#prospectingView').onsubmit=async e=>{
  ...
  if(e.target.id==='prospectLogForm'){
    ...
    prospectInteractions=prospectInteractions.map(x=>x.id===interactionId?{...x,metricsApplied:true}:x);
    try{await saveProspecting({render:false,awaitCloud:false})}catch(err){console.error('Prospecting log save failed',err);toast('The log was saved locally. Please check sync.')}clearProspectCallReturn(p.id);renderNowCard();renderTimeline();
