...
function setProspectorSection(section='today'){
  ...
  if(prospectSessionActive&&prospectSection==='today')return;
...
function saveKnockingSessionState(){try{localStorage.setItem(knockingSessionStorageKey(),JSON.stringify({active:knockingSessionActive,date:todayKey(),stats:knockingSessionStats,log:knockingSessionLog,startSeconds:knockingSessionStartSeconds,updatedAt:Date.now()}))}catch(err){console.warn('Knocking session state could not be saved',err)}}
...
function restoreKnockingSessionState(){try{const raw=JSON.parse(localStorage.getItem(knockingSessionStorageKey())||'null');if(!raw?.active)return;if(raw.date&&raw.date!==todayKey()){clearKnockingSessionState();return}knockingSessionStats={knocks:Number(raw.stats?.knocks)||0,clients:Number(raw.stats?.clients)||0,data:Number(raw.stats?.data)||0,MAP:Number(raw.stats?.MAP)||0,LAP:Number(raw.stats?.LAP)||0};knockingSessionLog=Array.isArray(raw.log)?raw.log.map(normaliseKnockingLogEntry):[];knockingSessionStartSeconds=Math.max(0,Number(raw.startSeconds)||0);knockingSessionActive=true;knockingSessionVisible=Boolean(dayData(todayKey()).timerStartedAt)}catch(err){console.warn('Knocking session state could not be restored',err);clearKnockingSessionState()}}
...
function dailyKnockingSessions(date=todayKey()){const d=dayData(date);return Array.isArray(d.knockingSessions)?d.knockingSessions:[]}
function dailyKnockingStats(date=todayKey()){const totals=dailyKnockingSessions(date).reduce((acc,session)=>{Object.keys(acc).forEach(key=>acc[key]+=Math.max(0,Number(session.stats?.[key])||0));return acc},{knocks:0,clients:0,data:0,MAP:0,LAP:0});if(knockingSessionActive&&date===todayKey())Object.keys(totals).forEach(key=>totals[key]+=Math.max(0,Number(knockingSessionStats[key])||0));return totals}
function dailyKnockingLog(date=todayKey()){const entries=[];dailyKnockingSessions(date).forEach(session=>(Array.isArray(session.log)?session.log:[]).forEach(entry=>entries.push({...normaliseKnockingLogEntry(entry),sessionId:String(session.id||''),activeSession:false})));if(knockingSessionActive&&date===todayKey())knockingSessionLog.forEach(entry=>entries.push({...normaliseKnockingLogEntry(entry),sessionId:'',activeSession:true}));const seen=new Set();return entries.filter(entry=>{if(seen.has(entry.id))return false;seen.add(entry.id);return true})}
function findKnockingLogRecord(id,date=todayKey()){const activeIndex=knockingSessionLog.findIndex(entry=>entry.id===id);if(activeIndex>=0)return{entry:knockingSessionLog[activeIndex],active:true,index:activeIndex,session:null,day:null};const d=dayData(date),sessions=Array.isArray(d.knockingSessions)?d.knockingSessions:[];for(const session of sessions){const index=Array.isArray(session.log)?session.log.findIndex(entry=>String(entry.id)===String(id)):-1;if(index>=0)return{entry:session.log[index],active:false,index,session,day:d}}return null}
...
async function updateKnockingLogEntry(entry,values){
  ...
  Object.assign(entry,values);const record=findKnockingLogRecord(entry.id,entry.createdDate);if(record?.active)saveKnockingSessionState();else if(record?.day){days[entry.createdDate]=record.day;await saveDay(entry.createdDate)}closeKnockingCapture();renderKnockingSession();toast('Session entry updated')
}
async function submitKnockingCapture(form){
  ...
  try{
    ...
    if(knockingEditingLogId){const record=findKnockingLogRecord(knockingEditingLogId,todayKey()),entry=record?.entry;if(!entry)throw new Error('Session entry not found');await updateKnockingLogEntry(entry,{name,phone,address,date,time});return}
...
async function deleteKnockingLogEntry(id){
  const record=findKnockingLogRecord(id,todayKey());if(!record)return;const {entry}=record;if(!confirm(`Delete this ${entry.type} session entry?`))return;
  if(entry.type==='data')await changeMetric('data',-1);
  if(record.active){if(entry.type!=='data'){const d=dayData(entry.createdDate);d.appointments=d.appointments.filter(a=>String(a.id)!==String(entry.appointmentId));days[entry.createdDate]=d;await saveDay(entry.createdDate)}knockingSessionStats[entry.type]=Math.max(0,(Number(knockingSessionStats[entry.type])||0)-1);knockingSessionLog.splice(record.index,1);saveKnockingSessionState()}
  else{const d=dayData(entry.createdDate),session=(Array.isArray(d.knockingSessions)?d.knockingSessions:[]).find(item=>String(item.id)===String(record.session.id))||findKnockingLogRecord(id,entry.createdDate)?.session;if(!session)return;if(entry.type!=='data')d.appointments=d.appointments.filter(a=>String(a.id)!==String(entry.appointmentId));session.stats={...session.stats,[entry.type]:Math.max(0,(Number(session.stats?.[entry.type])||0)-1)};session.log=(Array.isArray(session.log)?session.log:[]).filter(item=>String(item.id)!==String(id));days[entry.createdDate]=d;await saveDay(entry.createdDate)}renderKnockingSession();toast('Session entry deleted')
}
...
async function endKnockingSession(){
  if(!knockingSessionActive)return;const stats={...knockingSessionStats},log=knockingSessionLog.map(normaliseKnockingLogEntry),date=todayKey(),beforeStop=dayData(date),durationSeconds=Math.max(0,liveKnockSeconds(beforeStop)-knockingSessionStartSeconds);
  if(beforeStop.timerStartedAt)await toggleTimer();
  const d=dayData(date);d.timerStartedAt=null;d.knockingSessions=Array.isArray(d.knockingSessions)?d.knockingSessions:[];d.knockingSessions.push({id:uuid(),date,startedAt:Date.now()-durationSeconds*1000,endedAt:Date.now(),durationSeconds,stats,log});d.knockingSessions=d.knockingSessions.slice(-100);days[date]=d;await saveDay(date);
