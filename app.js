...
let targets={...DEFAULTS}, days={}, selectedDate=dateKey(new Date()), appointmentDate=selectedDate;
...
function loadLocal(){try{days=JSON.parse(localStorage.getItem('da-days-v5')||localStorage.getItem('da-days-v4')||'{}');targets={...DEFAULTS,...JSON.parse(localStorage.getItem('da-targets-v5')||localStorage.getItem('da-targets-v4')||'{}')}}catch{days={};targets={...DEFAULTS}}}
function saveLocal(){localStorage.setItem('da-days-v5',JSON.stringify(days));localStorage.setItem('da-targets-v5',JSON.stringify(targets))}
async function saveDay(k,{quiet=false}={}){const clean={...dayData(k),clientUpdatedAt:Date.now()};days[k]=clean;saveLocal();renderAll();if(!cloud)return;setSync('','Saving');try{await setDoc(doc(db,'users',uid,'days',k),{...clean,updatedAt:serverTimestamp()},{merge:true});setSync('live','Live')}catch(err){console.error(err);setSync('error','Sync error');if(!quiet)toast('Saved on this device. Cloud sync failed.')}}
async function saveTargets(){saveLocal();if(!cloud)return;setSync('','Saving');try{await setDoc(doc(db,'users',uid),{targets,updatedAt:serverTimestamp()},{merge:true});setSync('live','Live')}catch(err){console.error(err);setSync('error','Sync error');toast('Targets saved locally. Cloud sync failed.')}}
...
$('#localMode').onclick=()=>{cloud=false;uid='local';setSync('offline','This device');showApp()};
...
$('#saveSettings').onclick=async()=>{targets={calls:+$('#callsTarget').value||50,connects:+$('#connectsTarget').value||25,data:+$('#dataTarget').value||10,weeklyKnock:+$('#weeklyKnockTarget').value||240};await saveTargets();renderAll();toast('Settings saved')};
$('#signOut').onclick=async()=>{if(cloud)await firebaseSignOut(auth);location.reload()};
