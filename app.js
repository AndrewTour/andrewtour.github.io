...
function renderOffDayConversations(){
  ...
  host.innerHTML=items.map(item=>`<article class="off-day-conversation-row"><button class="off-day-conversation-profile" type="button" data-plan-action="${escapeHtml(item.action)}" data-event-id="${escapeHtml(item.eventId||'')}"><span class="off-day-conversation-copy"><span>${escapeHtml(item.role)}</span><strong>${escapeHtml(item.name)}</strong><small><b>Last</b>${escapeHtml(item.lastContact||'No contact recorded')}</small><small><b>Next</b>${escapeHtml(item.nextContact||item.reason)}</small></span><b class="off-day-conversation-chevron" aria-hidden="true">›</b></button>${offDayConversationCallMarkup(item)}</article>`).join('')
}
