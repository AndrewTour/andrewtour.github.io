...
const BUYER_MATCH_STATUSES=new Set(['active','reviewed','contacted','dismissed','superseded']);
...
function dailyPlanBuyerMatchWorkload(viewDate){
  if(viewDate!==todayKey())return null;const rows=allOpenBuyerPropertyMatches();if(!rows.length)return null;const buyerCount=new Set(rows.map(row=>row.buyer.id)).size,propertyCount=new Set(rows.map(row=>row.match.propertyKey)).size,duration=dailyPlanRoundMinutes(rows.length*5,15,45),matches=rows.map(row=>({buyerId:row.buyer.id,buyerName:row.buyer.name,matchId:row.match.id,address:row.match.address,suburb:row.match.suburb,eventType:row.match.eventType,reason:row.match.reason,phone:primaryProspectPhone(row.buyer)}));
  return{id:'buyer-matches',workloadId:'buyer-matches',kind:'buyer-match',title:`${propertyCount} new propert${propertyCount===1?'y matches':'ies match'} ${buyerCount} buyer${buyerCount===1?'':'s'}`,meta:`${rows.length} buyer-to-property match${rows.length===1?'':'es'} · ${duration} min protected`,kicker:`BUYER MATCHES · ${rows.length} READY`,duration,minimumSegment:15,clientCount:buyerCount,propertyCount,remainingClients:buyerCount,matches,completed:false,actionable:true,action:'view-buyer-matches',label:'Open buyers',commandTitle:`Connect ${buyerCount} buyer${buyerCount===1?'':'s'} to fresh stock`,commandMeta:`${propertyCount} MarketPulse propert${propertyCount===1?'y is':'ies are'} inside their verified brief${buyerCount===1?'':'s'} · ${duration} protected minutes.`,rank:-60};
}
...
function dailyPlanAssignBuyerMatches(items=[]){
  if(!items.length)return items;const remaining=[...(items[0].matches||[])];items.forEach((item,index)=>{const capacity=Math.max(1,Math.floor((Number(item.duration)||15)/5)),matches=remaining.splice(0,capacity),buyerCount=new Set(matches.map(match=>match.buyerId)).size,propertyCount=new Set(matches.map(match=>`${match.address}|${match.suburb}`)).size,overflow=index===items.length-1&&remaining.length?` · ${remaining.length} more in Buyers`:'';item.matches=matches;item.clientCount=buyerCount;item.propertyCount=propertyCount;item.title=`${propertyCount} propert${propertyCount===1?'y match':'ies match'} ${buyerCount} buyer${buyerCount===1?'':'s'}`;item.meta=`${matches.length} verified match${matches.length===1?'':'es'} · ${item.duration} min protected${overflow}`});return items
}
...
function timelineBuyerMatchesMarkup(matches=[]){
  if(!matches.length)return'';return`<div class="timeline-buyer-match-list" aria-label="Matched buyers and properties">${matches.map(match=>`<article><span><strong>${escapeHtml(match.buyerName)}</strong><small>${escapeHtml(match.address)}, ${escapeHtml(match.suburb)}</small><em>${escapeHtml(match.reason)}</em></span><div><button type="button" data-buyer-match-call="${escapeHtml(match.buyerId)}" data-match-id="${escapeHtml(match.matchId)}" ${match.phone?'':'disabled'}>Call</button><button type="button" data-buyer-match-sms="${escapeHtml(match.buyerId)}" data-match-id="${escapeHtml(match.matchId)}" ${match.phone?'':'disabled'}>SMS</button></div></article>`).join('')}</div>`
}
...
function renderTimeline(){
  ...
  const appointmentCount=items.filter(item=>item.kind==='appointment'||item.kind==='ofi').length,marketWorkloads=new Map();items.filter(item=>item.kind==='market').forEach(item=>{if(!marketWorkloads.has(item.sourceWorkloadId))marketWorkloads.set(item.sourceWorkloadId,item.remainingClients||0)});const marketClients=[...marketWorkloads.values()].reduce((sum,count)=>sum+count,0),buyerMatches=allOpenBuyerPropertyMatches().length,summary=[];if(buyerMatches)summary.push(`${buyerMatches} buyer match${buyerMatches===1?'':'es'}`);if(marketClients)summary.push(`${marketClients} MarketPulse client${marketClients===1?'':'s'}`);summary.push(`${appointmentCount} appointment${appointmentCount===1?'':'s'}`);summary.push(`${completion(selectedDate)}% complete`);$('#timelineSummary').textContent=summary.join(' · ');
...
function normaliseBuyerPropertyMatches(list=[]){
  ...
  return(Array.isArray(list)?list:[]).filter(item=>item&&typeof item==='object').map(item=>{
    ...
    return{id,eventId,propertyKey,eventType:cleanText(item.eventType,60),address:cleanText(item.address,240),suburb:cleanText(item.suburb,100),price:cleanText(item.price,120),guide:cleanText(item.guide,120),propertyDetails:cleanText(item.propertyDetails,240),propertyType:BUYER_PROPERTY_TYPES.includes(item.propertyType)?item.propertyType:'',bedrooms:Math.max(0,Number(item.bedrooms)||0),bathrooms:Math.max(0,Number(item.bathrooms)||0),cars:Math.max(0,Number(item.cars)||0),receivedDate:validDateKey(item.receivedDate)?item.receivedDate:todayKey(),matchedAt:Number(item.matchedAt)||Date.now(),status:BUYER_MATCH_STATUSES.has(item.status)?item.status:'active',statusAt:Number(item.statusAt)||0,reason:cleanText(item.reason,320)};
...
function buyerMarketEventMatch(buyer,event={}){
  if(!buyer||buyer.recordType!=='buyer'||buyer.archived||buyer.buyerStage==='Purchased'||!buyerMatchEventEligible(event))return null;
...
function buyerPropertyMatchRecord(buyer,event,match){
  const snapshot=match.snapshot;return{id:event.id,eventId:event.id,propertyKey:event.propertyKey||marketPropertyKey(event.address,event.suburb),eventType:event.eventType,address:event.address,suburb:event.suburb,price:event.price||'',guide:event.guide||'',propertyDetails:event.propertyDetails||'',propertyType:snapshot.propertyType,bedrooms:snapshot.bedrooms,bathrooms:snapshot.bathrooms,cars:snapshot.cars,receivedDate:event.receivedDate||todayKey(),matchedAt:Date.now(),status:'active',statusAt:0,reason:match.reason};
}
function buyerMatchAsEvent(match={}){return{id:match.eventId,eventType:match.eventType,address:match.address,suburb:match.suburb,propertyKey:match.propertyKey,price:match.price,guide:match.guide,propertyDetails:match.propertyDetails,receivedDate:match.receivedDate}}
function refreshBuyerPropertyMatches(events=marketPulseEvents){
  ...
  prospects=prospects.map(buyer=>{
    if(buyer.recordType!=='buyer'||buyer.archived||buyer.buyerStage==='Purchased')return buyer;
    const before=normaliseBuyerPropertyMatches(buyer.buyerPropertyMatches),next=before.map(item=>({...item}));
    next.forEach(item=>{if(item.status==='active'&&!buyerMarketEventMatch(buyer,buyerMatchAsEvent(item))){item.status='superseded';item.statusAt=Date.now()}});
    for(const event of incoming){
      const propertyKey=event.propertyKey||marketPropertyKey(event.address,event.suburb);if(!propertyKey)continue;
      const laterOpen=next.filter(item=>item.status==='active'&&item.propertyKey===propertyKey&&event.receivedDate>=item.receivedDate&&item.eventId!==event.id);
      ...
      if(existingMatch){if(existingMatch.status==='superseded'&&match)Object.assign(existingMatch,buyerPropertyMatchRecord(buyer,event,match));continue}
...
function buyerOpenPropertyMatches(buyer={}){
  ...
  normaliseBuyerPropertyMatches(buyer.buyerPropertyMatches).filter(item=>item.status==='active').sort((a,b)=>b.receivedDate.localeCompare(a.receivedDate)||b.matchedAt-a.matchedAt).forEach(item=>{if(!latest.has(item.propertyKey))latest.set(item.propertyKey,item)});
...
function buyerMatchSmsMessage(buyer={},match={}){const first=buyer.name.split(/\s+/)[0]||'there',config=[match.bedrooms?`${match.bedrooms} bedroom`:'',match.propertyType].filter(Boolean).join(' '),price=buyerMatchPriceLabel(match);return`Hi ${first}, a property has come up at ${match.address}, ${match.suburb} that matches what you’re looking for.${config?` It’s a ${config.toLowerCase()}.`:''}${price?` The guide is ${price}.`:''} Let me know if you’d like me to send through the details or arrange a look. ${displayAgentName().split(/\s+/)[0]||'Andrew'}`}
function setBuyerPropertyMatchStatusLocal(buyerId,matchId,status='reviewed',{logInteraction=true}={}){
  if(!['reviewed','contacted','dismissed'].includes(status))return null;const buyer=prospectById(buyerId);if(!buyer||buyer.recordType!=='buyer')return null;const selected=normaliseBuyerPropertyMatches(buyer.buyerPropertyMatches).find(item=>item.id===matchId);if(!selected)return null;const now=Date.now(),matches=normaliseBuyerPropertyMatches(buyer.buyerPropertyMatches).map(item=>item.status==='active'&&item.propertyKey===selected.propertyKey&&item.receivedDate<=selected.receivedDate?{...item,status,statusAt:now}:item),labels={reviewed:'Property match reviewed',contacted:'Property match contacted',dismissed:'Property match dismissed'};prospects=prospects.map(item=>item.id===buyerId?normaliseProspect({...item,buyerPropertyMatches:matches,updatedAt:now}):item);if(logInteraction)prospectInteractions.push({id:prospectId(),prospectId:buyerId,date:todayKey(),at:now,type:'Buyer match',outcome:labels[status],note:`${selected.address}, ${selected.suburb}${buyerMatchPriceLabel(selected)?` · ${buyerMatchPriceLabel(selected)}`:''}`,nextFollowUp:''});saveLocal();renderBuyerProfiles();renderTimeline();renderNowCard();if(activeProspectId===buyerId&&!$('#prospectDetail')?.classList.contains('hidden'))renderBuyerDetail(buyerId);saveProspecting({render:false,awaitCloud:false}).catch(err=>console.error('Buyer match status sync failed',err));return{buyer,match:selected}
}
function launchBuyerMatchCall(buyerId,matchId){launchBuyerProfileCall(buyerId,matchId)}
function launchBuyerMatchSms(buyerId,matchId){const buyer=prospectById(buyerId),match=normaliseBuyerPropertyMatches(buyer?.buyerPropertyMatches).find(item=>item.id===matchId),phone=primaryProspectPhone(buyer);if(!buyer||!match)return;if(!phone)return toast('Add a valid mobile number first');setBuyerPropertyMatchStatusLocal(buyerId,matchId,'contacted');window.location.href=smsHref(phone,buyerMatchSmsMessage(buyer,match))}
...
function buyerPropertyMatchesMarkup(p={}){
  ...
  return`<section class="buyer-property-matches"><div class="buyer-property-matches-head"><div><span>MARKETPULSE MATCHES</span><h3>${matches.length} propert${matches.length===1?'y':'ies'} worth discussing</h3></div><b>${matches.length}</b></div><div class="buyer-property-match-list">${matches.map(match=>{const config=[match.propertyType,match.bedrooms?`${match.bedrooms} bed`:'',match.bathrooms?`${match.bathrooms} bath`:''].filter(Boolean).join(' · '),price=buyerMatchPriceLabel(match);return`<article><span class="buyer-match-event">${escapeHtml(match.eventType)}</span><h4>${escapeHtml(match.address)}</h4><p>${escapeHtml(match.suburb)}${price?` · ${escapeHtml(price)}`:''}</p><small>${escapeHtml(config)}</small><em>${escapeHtml(match.reason)}</em><div><button type="button" data-buyer-match-call="${escapeHtml(p.id)}" data-match-id="${escapeHtml(match.id)}" ${phone?'':'disabled'}>Call</button><button type="button" data-buyer-match-sms="${escapeHtml(p.id)}" data-match-id="${escapeHtml(match.id)}" ${phone?'':'disabled'}>SMS</button><button type="button" data-buyer-match-status="reviewed" data-buyer-id="${escapeHtml(p.id)}" data-match-id="${escapeHtml(match.id)}">Reviewed</button><button type="button" data-buyer-match-status="dismissed" data-buyer-id="${escapeHtml(p.id)}" data-match-id="${escapeHtml(match.id)}">Dismiss</button></div></article>`}).join('')}</div></section>`
}
...
$('#prospectingView').onclick=async e=>{
  ...
  const buyerMatchStatus=e.target.closest('[data-buyer-match-status]');if(buyerMatchStatus){setBuyerPropertyMatchStatusLocal(buyerMatchStatus.dataset.buyerId,buyerMatchStatus.dataset.matchId,buyerMatchStatus.dataset.buyerMatchStatus);toast(buyerMatchStatus.dataset.buyerMatchStatus==='dismissed'?'Property match dismissed':'Property match reviewed');return}
