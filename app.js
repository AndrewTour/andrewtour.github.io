...
let knockingSessionActive=false,knockingSessionVisible=false,knockingSessionEnding=false,knockingSessionStats={knocks:0,clients:0,data:0,MAP:0,LAP:0},knockingSessionLog=[],knockingSessionVisits=[],knockingSessionStartSeconds=0,knockingCaptureType='',knockingEditingLogId='',selectedKnockingStreetKey='',selectedKnockingMarketKey='';
...
function setKnockingStreet(key){if(!key)return false;selectedKnockingStreetKey=cleanText(key,220);saveKnockingSessionState();renderKnockingStreetData();const summary=$('#knockingStreetSummary'),parts=knockingStreetParts(selectedKnockingStreetKey);if(summary)summary.textContent=`Showing ${parts.label}. Search remains available.`;return true}
function applyKnockingStreetInput(){const input=$('#knockingStreetSearch'),value=cleanText(input?.value,220);if(!value){selectedKnockingStreetKey='';saveKnockingSessionState();renderKnockingStreetData();const summary=$('#knockingStreetSummary');if(summary)summary.textContent='Search any known street and suburb.';return}const directory=knockingStreetDirectory(),normalised=normalisePlace(value),matched=directory.find(item=>normalisePlace(item.label)===normalised||item.key===value);if(matched)return setKnockingStreet(matched.key);const parsed=splitMarketAddress(value),street=marketStreetName(parsed.address||value),suburb=normalisePlace(parsed.suburb);if(!street||!suburb)return toast('Enter the street and suburb');return setKnockingStreet(`${street}|${suburb}`)}
function setKnockingMarketStreet(key){if(!key)return;selectedKnockingMarketKey=cleanText(key,220);renderKnockingHotSpotting()}
function renderKnockingHotSpotting(){
  const section=$('#knockingHotSpottingRecommendations'),host=$('#knockingHotSpottingList'),count=$('#knockingHotSpottingCount'),options=$('#knockingStreetOptions'),input=$('#knockingStreetSearch'),summary=$('#knockingStreetSummary'),priorityHost=$('#knockingPriorityStreets');if(!section||!host||!options||!input)return;
  ...
  if(!selectedKnockingMarketKey||!priorities.some(item=>item.key===selectedKnockingMarketKey))selectedKnockingMarketKey=priorities[0]?.key||'';
  const parts=knockingStreetParts(selectedKnockingMarketKey),events=knockingStreetEvents(selectedKnockingMarketKey),latestDate=events[0]?.receivedDate||'',latestEvents=events.filter(event=>event.receivedDate===latestDate),reasons=[...new Set(latestEvents.map(event=>cleanText(event.eventType,60)).filter(Boolean))],properties=latestEvents.map(event=>[event.address,event.price||event.guide].filter(Boolean).join(' · ')).filter(Boolean);
  priorityHost.innerHTML=priorities.length?priorities.map(item=>`<button type="button" data-select-marketpulse-street="${escapeHtml(item.key)}" class="${item.key===selectedKnockingMarketKey?'active':''}"><span>${escapeHtml(item.priority)} PRIORITY</span><strong>${escapeHtml(item.street)}</strong><small>${escapeHtml([item.suburb,...[...new Set(item.events.map(event=>event.eventType))]].filter(Boolean).join(' · '))}</small></button>`).join(''):'<p>No current MarketPulse street trigger.</p>';
  if(summary&&!selectedKnockingStreetKey)summary.textContent=`Search ${directory.length} known street${directory.length===1?'':'s'} independently.`;
  if(count)count.textContent=latestEvents.length;
  host.innerHTML=latestEvents.length?`<article class="knocking-hotspot-item selected"><div><strong>${escapeHtml(parts.label)}</strong><small>${escapeHtml(latestDate===todayKey()?'MarketPulse today':latestDate?fmtDate(latestDate):'Recent MarketPulse')}</small></div><div class="knocking-hotspot-reasons">${reasons.map(reason=>`<span class="market-event-tag event-${marketTypeClass(reason)}">${escapeHtml(reason)}</span>`).join('')}</div><p><strong>${escapeHtml(properties.slice(0,2).join(' · '))}</strong>${properties.length>2?`<br>+${properties.length-2} more property update${properties.length-2===1?'':'s'}`:''}</p></article>`:'<div class="knocking-market-empty"><strong>No current MarketPulse trigger</strong><small>Start the session using the metric controls below.</small></div>';
...
function renderKnockingStreetData(){
  const host=$('#knockingStreetData'),summary=$('#knockingStreetDataSummary');if(!host||!summary)return;const parts=knockingStreetParts(selectedKnockingStreetKey);
  ...
  summary.textContent=`${contacts.length} saved record${contacts.length===1?'':'s'} · ${visits.length} knock${visits.length===1?'':'s'}`;host.innerHTML=rows.length?rows.join(''):'<div class="knocking-log-empty">No previous conversations recorded for this street.</div>'
}
