...
let targets={...DEFAULTS}, days={}, prospects=[], prospectInteractions=[], prospectFilter='priority', prospectSection='today', prospectBulkMode=false, selectedProspectIds=new Set(), activeProspectId=null, prospectSessionIds=[], prospectSessionIndex=0, selectedDate=dateKey(new Date()), appointmentDate=selectedDate, appointmentHistoryMode=null, agentName='', calendarPreference='outlook', leaderboardEntries=[], leaderboardMode='day', leaderboardDayOffset=0, leaderboardWeekOffset=0, scorecardWeekOffset=0;
...
function prospectCard(p,{contactsView=false}={}){const initials=p.name.split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase();if(contactsView){const selected=selectedProspectIds.has(p.id),phone=primaryProspectPhone(p),address=p.address||'No property address';return`<button class="prospect-card contact-card-v156 ${prospectBulkMode?'bulk-mode':''} ${selected?'selected':''}" type="button" ${prospectBulkMode?`data-select-prospect="${p.id}" aria-pressed="${selected}"`:`data-open-prospect="${p.id}"`}>
...
function renderProspecting(){
  ...
  const queue=priorityProspects().filter(p=>p.nextFollowUp&&p.nextFollowUp<=today).slice(0,5);$('#prospectQueue').innerHTML=queue.length?queue.map(p=>prospectCard(p)).join(''):'<div class="prospect-empty"><strong>Queue cleared</strong><small>No overdue follow-ups. Your next opportunities are below.</small></div>';
