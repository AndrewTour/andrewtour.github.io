/* AGNT 1.44.19: independent startup protection; no authentication or data access. */
(()=>{
  'use strict';
  const version='1.44.19',key='agnt:runtime-events-v1',boot=Date.now().toString(36);
  const codes=new Set(['Error','TypeError','ReferenceError','RangeError','SyntaxError','SecurityError','QuotaExceededError','AbortError']);
  const events=new Set(['boot','ready','hidden','visible','pagehide','pageshow','pageshow-restored','online','offline','runtime-error','promise-error','module-error','startup-timeout','storage-error','render-error','worker-change','contact-save-start','contact-save-complete','contact-save-error']);
  ...
  function record(event,code){
    ...
    rows=rows.slice(-48);if(event.startsWith('contact-save-'))flush();else if(timer===null)timer=setTimeout(flush,300);
  }
